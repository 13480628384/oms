...
class logisticsmanager_ctl_admin_express_template extends desktop_controller{
    ...
    function index(){
        $base_filter = array('template_type' => array('normal', 'electron','cainiao'));
        $params = array(
            'title'=>'快递面单管理',
            'actions' => array(
                array(
                ...
                array(
                    'label' => '新增菜鸟面单',
                    'href' => 'index.php?app=logisticsmanager&ctl=admin_express_template&act=addTmpl&type=cainiao&finder_id='.$_GET['finder_id'],
                    'target' => "_blank",
                ),
                array(
                            'label' => '导入面单模板',
    ...
    function saveTmpl(){
        $data = array(
            ...
            'template_select' => $_POST['template_select'] ? serialize($_POST['template_select']) : null,
        ...
        if (!in_array($data['template_type'],array('normal', 'electron', 'delivery', 'stock','cainiao'))) {
            echo '面单类型不符合规则！';
            exit;
        }
