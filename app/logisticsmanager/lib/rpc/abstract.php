...
abstract class logisticsmanager_rpc_abstract {
    ...
    public function request($method, $params, $callback = array(), $shop_id, $writelog = array(), $time_out=30) {
        ...
        // 御城河
        /*
        if ($writelog['delivery_id'] && $this->channelType) {
            $tradeIds = array();
            $deliveryOrder = kernel::single('logisticsmanager_service_'.$this->channelType)->getDeliveryOrder($writelog['delivery_id']);
            foreach ((array) $deliveryOrder['order_info'] as $order_id => $orders) {
                $tradeIds[] = $orders['order_bn'];
            }
            kernel::single('base_hchsafe')->order_send_log($tradeIds,$params['to_node_id']);
        }
        */
