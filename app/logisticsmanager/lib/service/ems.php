...
class logisticsmanager_service_ems extends logisticsmanager_service_abstract{
    ...
    */
    public function delivery($delivery_id) {
        ...
        if($delivery['status']=='succ' && $delivery['logi_no'] && $channel['channel_type']=='ems') {
            ...
            if($emsAccount  && $consignee) {
                ...
                //记录补打物流单信息
                $bills = $this->delivery_bill($delivery['delivery_bn']);
    ...
    public function delivery_bill($delivery_bn) {

        $dlyObj = app::get('wms')->model('delivery');
        $delivery = $dlyObj->dump(array('outer_delivery_bn'=>$delivery_bn),'delivery_id');
       
        if ($delivery) {
            $deliveryBillObj = app::get('wms')->model('delivery_bill');
            $deliveryBillInfo = $deliveryBillObj->getlist('b_id,logi_no,status,weight',array('delivery_id'=>$delivery['delivery_id'],'type'=>2));
