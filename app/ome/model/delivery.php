...
class ome_mdl_delivery extends dbeav_model{
    ...
    public function _filter($filter,$tableAlias=null,$baseWhere=null){
        ...
        if(isset($filter['product_bn'])){
          ...
          //$where .= '  AND delivery_id IN ('.implode(',', $deliveryId).')';
            ...
            $where .= '  AND bnsContent like \'%'.$filter['product_bn'].'%\'';
    ...
    function rebackDelivery($dly_ids, $memo, $dly_status=false){
        ...
        foreach ($ids as $item){
            ...
            $deliveryInfo = $this->dump($item,'shop_id, process, status, delivery_bn, branch_id, is_bind, parent_id');
            ...
            if($res['rsp'] == 'success' || $res['rsp'] == 'succ'){
                ...
                foreach($product_ids as $key=>$v){
                    ...
                    #[增加]货品店铺冻结 ExBOY
                    $shopFreezeLib->freeze($deliveryInfo['shop_id'], $v['product_id'], $v['number']);
                    
                    #店铺冻结增减明细日志
                    if($is_shop_freeze_log)
                    {
                        $shopFreezeLogLib->changeLog($deliveryInfo['shop_id'], $v['product_id'], $v['number'], 23, '打回发货单号:'. $deliveryInfo['delivery_bn'] .',增加货品店铺冻结');
                    }
                ...
                if ($this->save($data)){
                    if (!$dly_status){
                        ...
                        $orderlist = $orderObj->getlist('order_id,order_combine_idx,order_combine_hash',array('order_id'=>$order));
                        ...
                        foreach($orderlist as $orderInfo) {
                            ...
                            if ($this->validDeiveryByOrderId($orderInfo['order_id']))
                            {
                                $opData['confirm'] = 'Y';
     ...
     */
    function validDeiveryByOrderId($order_id, $return_num=false){
    ...
    function addDelivery($order_id,$delivery,$ship_info=array(),$order_items=array(),&$split_status){
        ...
        if($delivery['type']!='reject'){//如果不为售后生成的发货单，才进行仓库货品的冻结 fix by danny 2012-4-26
            ...
            foreach($delivery['delivery_items'] as $key=>$items){
                ...
                #[释放]货品店铺冻结 ExBOY
                $shopFreezeLib->unfreeze($order['shop_id'], $items['product_id'], $items['number']);
                
                #店铺冻结增减明细日志
                if($is_shop_freeze_log)
                {
                    $shopFreezeLogLib->changeLog($order['shop_id'], $items['product_id'], $items['number'], 21, '新建发货单号:'. $delivery_bn .',释放货品店铺冻结');
                }
    ...
    function format_call_delivery_api($delivery_id, $fastConsign)
    {
        ...
        /*------------------------------------------------------ */
        $orderSplitLib    = kernel::single('ome_order_split');
        $chk_split = $orderSplitLib->check_order_split($delivery_id);
        ...
        else
        {
            ...
            if($split_type)
            {
                ...
                if(in_array($shop_type, array('ecos.b2c', 'ecos.dzg')))
                {
                    if($split_type == 1)
                    {
                        $get_dly_process    = $orderSplitLib->get_delivery_process($delivery_id, 'true');//第一次发货时,回写发货单
                    }
                    else
                    {
                        $get_dly_process    = $orderSplitLib->get_delivery_process($delivery_id, 'false');//全部发完货时,回写发货单
                    }
        ...
        if ( $iostock_sales_set_result )
        {
            ...
            if ($dly['is_bind'] == 'true'){ 
                ...
                foreach ($ids as $item){
                    ...
                    $get_dly_process    = $orderSplitLib->get_delivery_process($delivery['delivery_id'], 'false', $dly['delivery_id']);
                    if(empty($get_dly_process['delivery']) && empty($get_dly_process['order_items']))
                    {
                ...
                if (kernel::single('desktop_user')->get_id())
                {
                    $delivery_bn_str    = (empty($dly['delivery_bn']) ? '' : '（发货单号：'.$dly['delivery_bn'].'）');//日志增加_发货单号 ExBOY
                ...
                //判断订单是否全部发完货时 ExBOY
                $get_dly_process    = $orderSplitLib->get_delivery_process($dly['delivery_id'], 'false');
                if(empty($get_dly_process['delivery']) && empty($get_dly_process['order_items']))
                {
                    $orderdata['archive'] = 1;//订单归档
                }
