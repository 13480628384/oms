...
class ome_mdl_orders extends dbeav_model{
    ...
    */
    ...
    function pauseOrder($order_id, $must_update = 'false'){
        ...
        if ($order_id){
            $o = $this->dump($order_id,'pause, process_status, ship_status, shop_id');//[拆单]确认状态_发货状态 ExBOY

            #[拆单]部分拆分_部分发货_订单暂停操作(存在多个发货单)
            if(!empty($split_seting) && ($o['process_status'] == 'splitting' || $o['ship_status'] == '2'))
            {
                $rs     = $orderSplitLib->pauseOrder_split($order_id, $must_update);
                return $rs;
            }
                        ...
                        if($is_selfWms){
                            ...
                            if($res['rsp'] == 'success' || $res['rsp'] == 'succ'){
                                ...
                                foreach($product_ids as $key=>$v){
                                    ...
                                    {
                                        $shopFreezeLogLib->changeLog($o['shop_id'], $v['product_id'], $v['number'], 4, '第三方仓储取消发货单号:'. $deliveryInfo['delivery_bn'] .',增加店铺冻结');
                                    }
                                }
                                //
                            }else{
    ...
    function renewOrder($order_id){
        ...
        if ($order_id){
            ...
            if(!empty($split_seting) && ($o['process_status'] == 'splitting' || $o['ship_status'] == '2'))
            {
                $rs     = $orderSplitLib->renewOrder_split($order_id);
                return $rs;
            }
    ...
    function rebackDeliveryByOrderId($order_id,$dly_status=false){
        ...
        if(!empty($split_seting))
        {
            ...
            if(!empty($order_row))
            {
                $result     = $orderSplitLib->rebackDeliveryByOrderId_split($order_id, $dly_status);
                return $result;
            }
        }
    ...
    function unfreez($order_id){
        ...
        $orderRow   = $this->dump(array('order_id'=>$order_id), 'shop_id, order_bn');
        ...
        foreach($items as $v){
            ...
            #[释放]货品店铺冻结 ExBOY
            $shopFreezeLib->unfreeze($shop_id, $v['product_id'], $num);
            
            #店铺冻结增减明细日志
            if($is_shop_freeze_log)
            {
                $shopFreezeLogLib->changeLog($shop_id, $v['product_id'], $num, 10, '订单号:'. $orderRow['order_bn'] .'释放预占的店铺冻结');
            }
        }
    ...
    function cancel_delivery($order_id)
    {
        ...
        //拆单一个订单可能对应多个发货单
        $sql    = "SELECT dord.delivery_id,d.is_bind,d.status,d.delivery_bn,d.branch_id, d.shop_id FROM sdb_ome_delivery_order AS dord 
                   LEFT JOIN sdb_ome_delivery AS d ON(dord.delivery_id=d.delivery_id) 
                   WHERE dord.order_id={$order_id} AND d.disabled='false' AND d.parent_id=0 AND d.status not in('cancel','back','succ','return_back')";
        $deliveryList = $this->db->select($sql);
        ...
        foreach($deliveryList as $delivery)
        {
            ...
            if ($is_bind == 'false') {
                ...
                if ( $res['rsp'] == 'success' || $res['rsp'] == 'succ' ) {
                    ...
                    $product_ids = $delivery_itemsObj->getList('product_id,number',array('delivery_id'=>$delivery['delivery_id']),0,-1);
                    foreach($product_ids as $key=>$v){
                        $branch_productObj->unfreez($branch_id,$v['product_id'],$v['number']);
                        ...
                        if($is_shop_freeze_log)
                        {
                            $shopFreezeLogLib->changeLog($shop_id, $v['product_id'], $v['number'], 24, '发货单号:'. $delivery_bn .'取消,增加货品店铺冻结');
                        }
                        
                    }
                    $dlyObj->update_sync_cancel($delivery_id,'succ');
                    
                    $rs['succ_num']++;#取消发货单成功数量
                ...
                //合并发货单,请求的
                $SQL = "SELECT dord.delivery_id,dord.order_id, d.delivery_bn FROM sdb_ome_delivery_order AS dord
                                            LEFT JOIN sdb_ome_delivery AS d ON(dord.delivery_id=d.delivery_id)
                ...
                if ($delivery_order) {
                    $order_items = $itemsObj->getlist('product_id,nums',array('order_id'=>$delivery_order['order_id']));
                     foreach ( $order_items as $item ) {
                         $branch_productObj->unfreez($branch_id,$item['product_id'],$item['nums']);
                         
                         #[增加]货品店铺冻结 ExBOY
                         $shopFreezeLib->freeze($shop_id, $item['product_id'], $item['nums']);
                         
                         #店铺冻结增减明细日志
                         if($is_shop_freeze_log)
                         {
                             $shopFreezeLogLib->changeLog($shop_id, $item['product_id'], $item['nums'], 24, '合并发货单号:'. $delivery_order['delivery_bn'] .'取消,增加货品店铺冻结');
                         }
    ...
    function create_order(&$sdf){
        ...
        foreach($sdf['order_objects'] as $key=>$object){
            foreach($object['order_items'] as $k=>$item){
                ...
                //修改预占库存
                $item_quantity    = intval($item['quantity']) - intval($item['sendnum']);
                $oProducts->chg_product_store_freeze($item['product_id'], $item_quantity, "+");
                
                #[增加]货品店铺冻结 ExBOY
                if(!isset($item['delete']) || $item['delete'] == 'false')
                {
                    $shopFreezeLib->freeze($sdf['shop_id'], $item['product_id'], $item_quantity);
                    
                    #店铺冻结增减明细日志
                    if($is_shop_freeze_log)
                    {
                        $shopFreezeLogLib->changeLog($sdf['shop_id'], $item['product_id'], $item_quantity, 1, '订单号：' . $sdf['order_bn']);
                    }
                }
            }
        }
        ...
        {
            $orderSplitLib    = kernel::single('ome_order_split');
            $orderSplitLib->hold_order_delivery($sdf);
        }
        
        //-- 系统自动审单[订单必须已支付OR货到付款AND过滤单拉订单] ExBOY
        if(($sdf['pay_status'] == '1' || $sdf['shipping']['is_cod'] == 'true') && $sdf['status'] == 'active' && $sdf['auto_combine'] !== false)
        {
    ...
    */
    ...
    function presale_paymentadd($sdf){
        ...
        if ($presalesetting =='1' && $presaleconf =='1'){
            ...
            if ($result){
                $this->db->exec("UPDATE sdb_ome_orders SET payed=payed+".$money." WHERE order_id=".$sdf['order_id']);
            }

        }
        $order_detail = $this->dump($sdf['order_id'],'order_bn,payed,total_amount,shop_type, confirm, process_status, ship_status,source,is_cod');
        ...
        if ($pay_status){
            $this->db->exec("UPDATE sdb_ome_orders SET pay_status='".$pay_status."' WHERE order_id=".$sdf['order_id']);
        }
    }
