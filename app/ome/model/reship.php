...
class ome_mdl_reship extends dbeav_model{
    ...
    public function _filter($filter,$tableAlias=null,$baseWhere=null){
        ...
        if (isset($filter['order_bn'])){
            ...
            $archorder = $archive_ordObj->getOrder_list(array('order_bn|has'=>$filter['order_bn']),'order_id');
  ...
  function create_treship($adata,&$msg)
  {
     ...
     if ($adata['return_type'] == 'change') {
         $sdf_data['changebranch_id'] = $adata['changebranch_id'];
     }
     if ($adata['source'] == 'archive') {
          $sdf_data['archive'] = '1';
      ...
      if($sdf_data['reship_id']) {
          ...
          //判断是否为待确认，换货改为退货类型
          $reship_detail = $reshipObj->dump($sdf_data['reship_id'],'return_type,is_check,need_sv,changebranch_id');
          if ($reship_detail['return_type'] =='change' && $reship_detail['is_check']=='11' && $reship_detail['need_sv']=='true') {
               if($sdf_data['return_type']=='return'){
                   //释放冻结库存
                   kernel::single('console_reship')->change_freezeproduct($sdf_data['reship_id'],'-',$reship_detail['changebranch_id']);
               }
      ...
      if ($adata['reship_id'] && $adata['return_type'] == 'change') {
          ...
          $oldchange = $rchangeobj->getChangelist($adata['reship_id'],$adata['changebranch_id']);
      }
    ...
    if (isset($param['objects'])) {
        foreach ( $param['objects'] as $changeobj ) {
            ...
            if ($obj_type == 'pkg') {
                ...
                $objresult = $shipObj->save($obj);
            }else{
                if ($changeobj['item_id']) {
                    $obj['item_id'] = $changeobj['item_id'];
                }
                $obj['return_type'] = 'change';
                $itemsObj->save($obj);
            }
            $item_list = $changeobj['items'];
            if ($objresult) {
                $obj_id = $obj['obj_id'];
                foreach ( $item_list as $items ) {
                    $item = array();
                    $item['obj_id'] = $obj_id;
                    $item['price']       = $items['price']; //product_name
                    $item['num']         = $items['num'];
                    $bn = $items['bn'];
                    $item['product_id']  = $items['product_id'];
                    $item['return_type'] = 'change';
                    $item['op_id']       = $items['op_id'];
                    $item['product_name']       = $items['name'];
                    $item['reship_id']   = $reship_id;
                    $item['bn'] = $bn;
                    $item_detail = $itemsObj->dump(array('obj_id'=>$obj_id,'bn'=>$bn),'item_id');
            ...
            foreach ($reship_item as $key => $value) {
                ...
                if($value['return_type'] == 'return'){
                    if ($reship_data['archive']=='1' || ($reship_data['source'] && in_array($reship_data['source'],array('archive')))) {
                        ...
                        $refund = $archive_ordObj->Get_refund_count( $reship_data['order_id'], $value['bn'] ,$reship_id);
                    }else{
   ...
   */
   ...
   function create_order($reshipinfo,$reship_items)
   {
        ...
        $item_cost = 0;
        foreach ( $reship_items as &$items ) {
            unset($items['reship_id'],$items['type'],$items['branch_id'],$items['item_id'],$items['obj_id']);
            $items['obj_alias'] = $items['obj_type'] = $items['item_type'] == 'pkg' ? 'pkg': 'goods';
            $items['goods_id'] = $items['item_id'];
            $items['name'] = $items['product_name'];
            $items['quantity'] = $items['num'];
            $items['amount']=$items['sale_price'] = $items['num']*$items['price'];

            if ($items['item_type'] == 'pkg') {
                $order_items = $items['items'];
                foreach ($order_items as &$ov ) {
                    unset($ov['item_id'],$ov['obj_id']);
                    $ov['goods_id'] = $items['goods_id'];
                    $ov['name'] = $ov['product_name'];
                    $ov['quantity'] = $ov['num'];
                    $ov['amount']=$ov['sale_price'] = $ov['num']*$ov['price'];
                }
    ...
    function validate($data,&$v_msg){
        ...
        if($data['return_type'] == 'change'&& (!$type_change['objects']) ){
            $v_msg = '请至少选择一个换出商品。';
            return false;
        }
        ...
        if($data['return_type'] == 'change'&&$type_change['objects']){
           ...
           foreach ($type_change['objects'] as $objects ) {
               ...
               if ($obj_type == 'product') {
                   ...
                   $num = $objects['num'];
                   $sale_store = $bProductObj->getAvailableStore($changebranch_id,array($product_id));
                   $sale_store = $sale_store[$product_id];
                   
                   if ($num<1) {
                        $v_msg = '换出商品中,货号为:['.$objects['bn'].']申请数量为0，申请被拒绝。';
                        return false;
                   }
                   if ($sale_store< 1) {
                       $v_msg = '换出商品中,货号为:['.$objects['bn'].']实际的库存为0，申请被拒绝。';
                        return false;
                   }
                   if ($num>$sale_store) {
                       $v_msg = '换出商品中,货号为:['.$objects['bn'].']申请数量大于实际的库存。申请被拒绝。';
                        return false;
                   }
               }
               if ($obj_type == 'pkg') {
                   
                   foreach ($objects['items'] as $items ) {
                       $product_id = $items['product_id'];
                       $sale_store = $bProductObj->getAvailableStore($changebranch_id,array($product_id));
                       $sale_store = $sale_store[$product_id];
                       $num = $items['num'];
                       if ($num<1) {
                            $v_msg = '换出商品中,货号为:['.$items['bn'].']申请数量为0，申请被拒绝。';
                            return false;
                       }
                       if ($sale_store< 1) {
                           $v_msg = '换出商品中,货号为:['.$items['bn'].']实际的库存为0，申请被拒绝。';
                            return false;
                       }
                       if ($num>$sale_store) {
                           $v_msg = '换出商品中,货号为:['.$items['bn'].']申请数量大于实际的库存。申请被拒绝。';
                            return false;
                       }
                   }
               }
    ...
    function finish_aftersale($reship_id){
      ...
      $Reshipitem = $oReship_items->getList('bn,num,normal_num,defective_num',array('reship_id'=>$reship_id,'return_type'=>'return'));
         if ($is_archive) {
            $archive_ordobj = kernel::single('archive_interface_orders');
            $orders = $archive_ordobj->getOrders(array('order_id'=>$order_id),'*');
            kernel::single('archive_reship')->finish_aftersale($Reshipitem,$order_id);
         }else{
            foreach($Reshipitem as $k=>$v){
                //$orderItems = $oItemModel->getList('sendnum,bn,item_id',array('order_id'=>$order_id,'bn'=>$v['bn'],'sendnum|than'=>'0'));
                    ...
                    $itemsql = "SELECT sendnum,bn,item_id, return_num FROM sdb_ome_order_items 
                                                    WHERE order_id='".$order_id."' AND bn='".$v['bn']."' AND sendnum != return_num AND `delete`='false'";
             ...
             $order_sum = $this->db->selectrow("SELECT sum(sendnum) as count FROM sdb_ome_order_items WHERE order_id='".$order_id."' AND sendnum != return_num AND `delete`='false'");
              
              #拆单_部分发货_部分退货时_判断是否有未发货的货品 ExBOY
              $orders    = $oOrder->dump(array('order_id'=>$order_id));
              if(intval($order_sum['count']) == 0)
              {
                  if($orders['ship_status'] == '2' || $orders['ship_status'] == '3')
                  {
                     $sql    = "SELECT sum(nums - sendnum) as count FROM sdb_ome_order_items WHERE order_id = '".$order_id."' AND nums != sendnum AND `delete` = 'false'";
      ...
      $orders['ship_status']    = $ship_status;#更新后的_发货状态 ExBOY
      ...
      if($reshipinfo['return_type'] =='change'){
        ...
        if ($change_order_sdf) {
          $memo .=' 生成了1张换货订单【'.$change_order_sdf['order_bn'].'】';
            kernel::single('console_reship')->change_freezeproduct($reship_id,'-',$reshipinfo['changebranch_id']);//生成订单后释放库存
        # 源始订单记录关系订单号
      ...
      if($reshipinfo['return_id']){
       ...
       if ($change_order_sdf) {
        ...
        kernel::single('ome_service_aftersale')->update_status($reshipinfo['return_id'],'','',$newmemo);
      }
      $oOperation_log->write_log('reship@ome',$reship_id,$memo);
            ...
            if ($delitem) {
                foreach ($delitem as $dv ) {
                    if ($dk=='pkg') {
                        ...
                        $db->exec("DELETE FROM sdb_ome_reship_items WHERE reship_id=".$reship_id." AND item_id=".$dv);
                    }
                }
            }
