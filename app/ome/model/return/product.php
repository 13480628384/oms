    ...
    */
    function tosave($adata,$api=FALSE){
     ...
     function create_treship($return_id,$status)
     {
        ...
        //判断退货单重复
        if ($return_id){
            $reshipdata =$Oreship->dump(array('return_id'=>$return_id),'reship_id');
            if ($reshipdata){
                return false;
            }
        }
        ...
        if ($archiveLib->is_archive($source)) {
            ...
            $deliveryinfo = $archive_delObj->getDelivery(array('delivery_id'=>$returninfo['delivery_id']),'*');
        }else{
