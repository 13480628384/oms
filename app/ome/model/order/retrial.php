...
class ome_mdl_order_retrial extends dbeav_model
{
    ...
    public function getlist($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null)
    {
        ...
        $sql      = "SELECT a.*, b.total_amount, b.process_status, b.is_cod, b.pay_status, b.ship_status, b.payment, b.createtime, paytime FROM ". DB_PREFIX ."ome_order_retrial as a 
    ...
    /**
     +----------------------------------------------------------
     * 获取[创建订单后]的所有商品id及购买量
     +----------------------------------------------------------
     * @param   Array     $order_objects
     * return   Array
     +----------------------------------------------------------
     */
    public function get_create_order_product($order_objects)
    {
        $product_list  = array();
        $quantity_list = array();

        foreach ($order_objects as $key => $obj_item)
        {
            foreach ($obj_item['order_items'] as $key_j => $item)
            {
                $product_list['ids'][$item['product_id']]    = $item['product_id'];
                
                $quantity_list[$item['product_id']][]        = intval($item['quantity']);
            }
        }
    ...
    public function get_price_monitor($product_ids, $buy_product_nums=array(), $detail=false)
    {
        ...
        $oProduct   = $products->getList('product_id, goods_id, bn, cost, price', array('product_id|in'=>$product_ids));
        foreach ($oProduct as $key => $val)
        {
            ...
            if(!empty($buy_product_nums[$val['product_id']]))
            {
        ...
        foreach($product_list as $key => $val)
        {
            ...
            if($val['is_old'] == 'true')
            {
                ...
                if($val['status'] == 'delete')
                {
                    ...
                    {
                        $pObj->chg_product_store_freeze($product_id, $last_buy_num, '+');
                    }
                    else
                    {
                        $pObj->chg_product_store_freeze($product_id, $old_buy_num, '+');
                    }
                }
                elseif($val['status'] == 'edit')
                {
                    ...
                    if($last_product[$key]['status'] == 'delete')
                    {
                        ...
                        if($diff_buy_num > 0)
                        {                           
                            $pObj->chg_product_store_freeze($product_id, $diff_buy_num, '+');
                        }
                    }
                    else
                    {
                        ...
                        {
                            $pObj->chg_product_store_freeze($product_id, abs($diff_buy_num), '+');
                        }
                        
                        //减去上一次强制增加的库存
                        if($diff_up_num > 0)
                        {
                            $pObj->chg_product_store_freeze($product_id, $diff_up_num, '-');
                        }
                    }
                }
            }
    ...
    public function recover_stock_freeze($retrial_id)
    {
        ...
        foreach ($freeze_list as $key => $val)
        {
            ...
            if($val['is_old'] == 'true')
            {
                ...
                {
                    $pObj->chg_product_store_freeze($product_id, abs($num), '-');//减少
                }
            }
            else
            {
                ...
                {
                    $pObj->chg_product_store_freeze($product_id, abs($buy_num), '-');//减少
                }
            }
        }
    ...
    public function confirm_stock_freeze($retrial_id)
    {
        ...
        foreach ($freeze_list as $key => $val)
        {
            ...
            if($val['is_old'] == 'true')
            {
                ...
                {
                    $pObj->chg_product_store_freeze($product_id, abs($old_num), '-');//减少
                }
            }
        }
        $oFreeze->delete(array('retrial_id'=>$retrial_id));//清空库存记录
     ...
     * return   Array
     +----------------------------------------------------------
     */
    public function order_monitor($order)
    {
        #价格监控[获取订单复审配置]
        $setting_is_monitor = app::get('ome')->getConf('ome.order.is_monitor');//是否开启价格监控
        $cost_multiple      = app::get('ome')->getConf('ome.order.cost_multiple');//成本价倍数
        ...
        #成本价_销售价[监控]
        $product_list   = $this->get_create_order_product($order['order_objects']);
        $product_ids    = $product_list['ids'];
        $product_nums   = $product_list['nums'];
        
        $price_monitor  = $this->get_price_monitor($product_ids, $product_nums);
        $price_monitor['cost']  = floatval($price_monitor['cost']) * floatval($cost_multiple['value']);
        $price_monitor['cost']  = round($price_monitor['cost'], 2);

        #计算预计盈亏
        $profit     = ($total_amount - $price_monitor['cost']);
        
        if($profit < 0)
        {
            $retrial_msg       = '系统监控到订单销售价格有异常，自动进入价格复审';
            $oOperation_log    = app::get('ome')->model('operation_log');
            ...
            $retrial_arr['order_id']       = $order['order_id'];
            ...
            //写日志
            $oOperation_log->write_log('order_retrial@ome', $order['order_id'], $retrial_msg);
