...
class ome_ctl_admin_return extends desktop_controller {
    ...
    function save($status) {
        ...
        if ($archiveLib->is_archive($source)) {
            ...
            $order_detail = $archive_ordObj->getOrders(array('order_id'=>$oPro_detail ['order_id']),'ship_status');
        }else{
    ...
    */
    ...
    function apply_add() {
        ...
        // 获取单价
        $price = $oOrder_item->getList('price', array('order_id'=>$_POST['order_id']), 0, 1);
    ...
    function choose_type($return_id,$status)
    {
       ...
       //判断类型
        ...
        $this->pagedata['return_type'] = $bbc_detail['return_type'];
    ...
    function getOrders()
    {
        ...
        if (in_array($source,array('archive'))) {
            ...
            if (in_array($_SERVER['SERVER_NAME'],array('bz.erp.taoex.com'))) {
