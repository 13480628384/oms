...
class ome_ctl_admin_goods_editor extends desktop_controller{
    ...
    function nospec($cat_id=0){
        ...
        $oGoods = $this->app->model('products');
        $goods = $oGoods->dump(array('goods_id'=>$goods_id), 'bn,barcode');
        //$this->pagedata['product'] = $goods;
...
//                    $mprice[] = array( 'level_id'=>$mLvId,'price'=>$mLvPrice );
    ...
    function toAdd(){
       ...
       // $oGoods = $this->app->model('goods');
        ...
        $i = 0;
         if ($goods['goods_id']){
            $goods_detail = $oGoods->dump($goods['goods_id'],'createtime');
            $goods_createtime =$goods_detail['createtime'];
            unset($goods_detail);
        }
        foreach($_POST['goods']['product'] as $k=>$v){
            $v['bn'] = trim($v['bn']);
            ...
            if($v['bn'] == ''){
             ...
             //校验货号条码商品名称
            if ((($goods['goods_id'] && $goods_createtime>=1449025471) || !$goods['goods_id']) && !$productlib->validstr($v['bn'])){
                $this->begin('index.php?app=ome&ctl=admin_goods_editor&act=add');
                $this->end(false,app::get('base')->_('货号不可以含+等特殊字符！'));
            }
            if ($v['barcode']){
                if ((($goods['goods_id'] && $goods_createtime>=1449025471) ||  !$goods['goods_id']) && !$productlib->validstr($v['barcode'])){
                    $this->begin('index.php?app=ome&ctl=admin_goods_editor&act=add');
                    $this->end(false,app::get('base')->_('条码不可以含+等特殊字符！'));
                }
            }
      ...
      if($goods_detail){
        ...
        #商品保存后更新捆绑名称
        kernel::single('omepkg_ome_goods')->after_save($goods);
        #
        # 商品日志添加
...
//        $this->display('admin/goods/detail/level_price.html');
    ...
    function checkedGoodsInfo($bn = null){
        ...
        #检测商品有没有捆绑商品记录
       $_pkgInfo = $oGoods->checkPkgGoods($bn);
        if($_pkgInfo['num']>0){
           return  $msg = "该商品存在捆绑商品，货号不能再编辑";
        } 
        #检测商品有没有采购记录
