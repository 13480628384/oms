...
class ome_ctl_admin_dly_corp extends desktop_controller {
    ...
    function add(){
        ...
        if (app::get('logisticsmanager')->is_installed()) {
            ...
            $normalTmpl = $cainiaoTmpl =  $electronTmpl = array();
            foreach($templates as $val){
                if ($val['template_type']=='normal') {
                    $normalTmpl[] = $val;
                } elseif($val['template_type']=='cainiao') {
                    $cainiaoTmpl[] = $val;
            ...
            foreach($channel as $key=>$val) {
                ...
                }elseif ($val['channel_type'] == 'sf') {
                    $serviceType = logisticsmanager_waybill_sf::getLogiCodeByCarrierService($val['logistics_code']);
                    if($serviceType){
                        $channelType[$val['channel_id']] = $serviceType;
                    }
                }else{
    ...
    function saveDlType() {
        ...
        if (app::get('logisticsmanager')->is_installed()) {
            ...
            }elseif($dlycorp['tmpl_type']=='electron' && $dlycorp['electron_type']=='electron' ){
