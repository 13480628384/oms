...
//        echo "<script>$$('.dialog').getLast().retrieve('instance').close();</script>";
    ...
    function do_confirm($order_id, $oId = null) {
        ...
        $split_seting     = $orderSplitLib->get_delivery_seting();        $split_model    = 0;
        ...
        if($split_model)
        {
            ...
            {
                header("content-type:text/html; charset=utf-8");
                echo "<script>alert('捆绑商品：". implode(',', $fail_pkg['bn']) ." 存在删除状态的货品,请修正后再审核');opener.finderGroup['{$finder_id}'].refresh.delay(100,opener.finderGroup['{$finder_id}']);window.close();</script>";
                exit;
            }
            unset($fail_pkg);
    ...
    function abnormal(){
        ...
        $this->finder('ome_mdl_orders',array(
           'title'=>'异常订单',
           'base_filter'=>array('abnormal'=>'true','is_fail'=>'false','archive'=>0, 'process_status|notin'=>array('is_declare', 'is_retrial')),#过滤复审、跨境申报类型订单 ExBOY
           'use_buildin_new_dialog' => false,
           'use_buildin_set_tag'=>false,
           'use_buildin_recycle'=>false,
           'use_buildin_export'=>false,
...
//        error_log(var_export($_POST,true),3,__FILE__.".log");
        ...
        foreach($rows as $k=>$v) {
            ...
            //过滤京东电子面单  (拍拍使用京东面单)
            if ($shop_type != '360buy' && $shop_type != 'paipai' && $v['tmpl_type']=='electron' && $channelType[$v['channel_id']]=='360buy' && $v['shop_id']!=$shop_id) {
                continue;
            }
