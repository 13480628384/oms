...
class ome_ctl_admin_return_rchange extends desktop_controller {
    ...
    function add_rchange(){
        ...
        $post['change'] = $rchangeObj->format_Rchangedata($post['change']);
    ...
    function get_data($order_id,$type,$source=''){
        ...
        $archive_ordObj = kernel::single('archive_interface_orders');
        if ($source == 'archive') {
            $order_object = $archive_ordObj->getOrder_object(array('order_id'=>$order_id,'obj_type'=>'pkg'),'*');
    ...
    */
    ...
    function save_check($reship_id,$status,$is_anti = false)
    {
       ...
       if($reship_id){
            ...
            $reship = $Oreship->dump(array('reship_id'=>$reship_id),'order_id, is_check,reship_bn,return_type,reason,need_sv,source,changebranch_id');
            ...
            if ((($reship['is_check']=='0' && $reship['need_sv'] == 'true') || $reship['is_check']=='12') && ($reship['return_type'] == 'change')){
                ...
                foreach ( $change_item as $item ) {
                    $usable_store = $bProductObj->getAvailableStore($changebranch_id,array($item['product_id']));
            ...
            if (($reship['is_check']=='0' && $reship['need_sv'] == 'true') || $reship['is_check']=='12') {#发起通知单
                ...
                if ($reship['return_type'] == 'change' ) {
                    kernel::single('console_reship')->change_freezeproduct($reship_id,'+',$reship['changebranch_id']);
                }
    ...
    function getProducts(){
        ...
        $branch_id = $_GET['changebranch_id'];
        $Obranch = app::get('ome')->model('branch');
        ...
        if (!empty($data)){
            foreach ($data as $k => $item){
                ...
                $item['sale_store'] = $sale_store[$product_id] ? $sale_store[$product_id] : 0;
    ...
    function do_cancel($reship_id)
    {
        ...
        $reship = $Oreship->dump(array('reship_id'=>$reship_id),'reship_bn,return_id,return_type,changebranch_id,is_check');
        if($_POST){
    ...
    public function calDiffAmount($reship_id,$return_type)
    {
        ...
        if ($post['order_id']) {
            ...
            if ($source && in_array($source,array('archive'))) {
                ...
                $orderInfo = $archive_ordObj->getOrder_list(array('order_id'=>$post['order_id']),'total_amount,payed');
    ...
    function getPkgGoods(){
        ...
        if ($rows){
            foreach ($rows as $v){
                ...
                $v['price'] = '0';
                $items = array();
                $products = $productObj->getList('*',array('goods_id'=>$v['goods_id']),0,-1);
                foreach ($products as $product ) {
                    $sale_store = $bProductObj->getAvailableStore($branch_id,array($product['product_id']));
                    $items[] = array(
                        'product_id'=>$product['product_id'],
                        'bn'=>$product['bn'],
                        'name'=>$product['name'],
                        'num'=>$product['pkgnum'],
                        'sale_store'=>$sale_store[$product['product_id']],
                        'price'=>'0',
                    );
                }
                $v['items'] = $items;
     ...
     * @author sunjing@shopex.cn
     */
    function getchangeProducts($reship_id)
    {
