...
//        $consign = app::get('ome')->getConf('ome.delivery.consign');
    ...
    private function syncOrder($order_id) {
        ...
        if ($order) {
            ...
            #拆单_防止订单对应多个发货单时_有未打印、未校验、未发货的发货单 ExBOY
            if(count($deliveryIds) > 1 || $order['process_status']=='splitting' || $order['ship_status']=='2')
            {
                $delivery_id_list    = $deliveryObj->getList('delivery_id, status, logi_no, process, verify, expre_status', array('delivery_id'=>$deliveryIds));
            
                $delivery_ids    = array();
                foreach ($delivery_id_list as $key => $val)
                {
                    if(in_array($val['status'], array('progress', 'ready')))
                    {
                        continue;#等待配货
                    }
            
                    if($val['logi_no'] == '' || $val['process'] != 'true' || $val['verify'] != 'true')
                    {
                        continue;#物流单号为空、未发货、未校验
                    }
                ...
                unset($delivery_id_list, $delivery_ids);
            }
            
            if(!empty($deliveryIds))
            {
                foreach ($deliveryIds as $key => $delivery_id)
                {
                    $this->syncOrderDelivery($delivery_id, $order);
                }
            }
...
//    }
        ...
        */
        ...
        if ($weightSet=='off') {
            ...
            foreach($dly['delivery_order'] as $item){
                ...
                {
                    $orderWeight  = $orderSplitLib->getDeliveryWeight($item['order_id'], array(), $dly['delivery_id']);
                }
                else 
                {
        ...
        if(count($delivery)>0 && $group>0){
            ...
            foreach($deliverys as $value){
                ...
                foreach($delivery_order as $item){
                  ...
                  if(!empty($split_seting))
                  {
                      $orderWeight  = $orderSplitLib->getDeliveryWeight($item['order_id'], array(), $value['delivery_id']);
                  }
                  else 
                  {
 ...
 /**
    ...
    */
    ...
    /**
     * 对指定订单进行发货处理(开启拆单功能,防止订单对应多个发货单时)
     *
     * @author:ExBOY
     * @param $delivery_id  Intval 发货单Id
     * @param $order        Array  订单详情
     * @return boolean
     */
    function syncOrderDelivery($delivery_id, $order)
    {
        $dlyCorpObj = app::get('ome')->model('dly_corp');
        ...
        unset($order, $order_id, $delivery_id);
        return true;
    }
}
?>
