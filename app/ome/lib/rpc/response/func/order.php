...
* @copyright shopex.cn 2012.06.06
*/
class ome_rpc_response_func_order{
            ...
            if(in_array($shop_type,$jingdong_type)){
                ...
                if($process_status == 'cancel' || $status == 'dead'){
                    ...
                    if($pay_status == '0' && $ship_status == '0'){
                        ...
                        $dead_rs = $oOrder->cancel($order_id,$memo,true,'async');
                    }
                    if ($dead_rs['rsp'] == 'fail'){
        ...
        if( $local_pay_status != $pay_status ){
            ...
            if( in_array($local_pay_status, array('6','7')) && in_array($pay_status, array('1','3','4','5')) ){
                ...
                if($pay_status == '4'){
                    ...
                    $refund_applyObj->check_iscancel($order_id);
    ...
    */
    public function update_items($order_id,$sdf=array(),&$orders,&$is_change_goods,&$noexists_product){
        ...
        $compre_items = array('name','cost','price','amount','pmt_price','sale_price','weight','nums','item_type','score','delete','addon');
            ...
            if (isset($old_objects[$obj_key])){#obj存在
                ...
                foreach ($obj_val['order_items'] as $items_val){
                    ...
                    if (!isset($old_objects[$obj_key]['order_items'][$items_key])){
                        #前端订单商品存在于TG商品列表中
                        if ($p = $oProducts->getList('product_id',array('bn'=>$items_val['bn']),0,1)){
                            $product_id = $p[0]['product_id'];
                            $oProducts->chg_product_store_freeze($product_id,intval($items_val['quantity']),"+");
                        ...
                        if ($items_val['status'] == 'close' && $tg_status == 'false'){
                            ...
                            if ($orders['is_fail'] == 'true'){
                                if ($tg_product_id > 0){
                                    $oProducts->chg_product_store_freeze($tg_product_id,$tg_nums,"-");
                                }
                            }else{
                                $oProducts->chg_product_store_freeze($tg_product_id,$tg_nums,"-");
                            }
                            ...
                            if ($orders['is_fail'] == 'true'){
                                if ($tg_product_id > 0){
                                    $oProducts->chg_product_store_freeze($tg_product_id,$items_val['quantity'],"+");
                                }
                            }else{
                                $oProducts->chg_product_store_freeze($tg_product_id,$items_val['quantity'],"+");
                            }
                            ...
                            if ($orders['is_fail'] == 'true'){
                                if ($tg_product_id > 0 && $diff_nums != 0){
                                    if ($diff_nums > 0){
                                        $oProducts->chg_product_store_freeze($tg_product_id,$diff_nums,"+");
                                    }else{
                                        $oProducts->chg_product_store_freeze($tg_product_id,abs($diff_nums),"-");
                                    }
                                }
                            }else{
                                if ($diff_nums != 0){
                ...
                foreach ($obj_val['order_items'] as &$add_items_val){
                    ...
                    if ($p = $oProducts->getList('product_id',array('bn'=>$bn),0,1)){
                        #前端订单商品存在于TG商品列表中
                        $product_id = $p[0]['product_id'];
                        $oProducts->chg_product_store_freeze($product_id,$add_items_val['quantity'],"+");
                ...
                if($obj_val['bn']){
                    $goods_info = $oGoods->getList("goods_id",array('bn'=>$obj_val['bn']),0,1);
                    if($goods_info){
                        $obj_val['goods_id'] = $goods_info['goods_id'];
                    }
                }
    ...
    */
    ...
    public function order_objects_filter($order_bn='',$shop_id='',$shop_name='',&$order_objects){
        ...
        foreach($new_order_objects as $key=>$object){
            //子订单
            if($object['bn']){
                $goods_info = $oGoods->getList("goods_id",array('bn'=>$object['bn']),0,1);
                if($goods_info){
                    $order_objects[$key]['goods_id'] = $goods_info[0]['goods_id'];
                }
            }
            ...
            foreach($items as $k=>$item){
                //判断货号是否存在
                $sql = "SELECT `product_id`,`bn` FROM `sdb_ome_products` WHERE `bn`='".$item['bn']."' ";
