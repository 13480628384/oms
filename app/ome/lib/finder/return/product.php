...
class ome_finder_return_product{
    ...
    function detail_basic($return_id){
        ...
        if ($is_archive || $product_detail['archive']=='1') {
            ...
            $order_detail = $oOrder->dump(array('order_id'=>$product_detail['order_id']));            $archive_ordObj = kernel::single('archive_interface_orders');
            $order_detail = $archive_ordObj->getOrders(array('order_id'=>$product_detail['order_id']),'*');        }else{
