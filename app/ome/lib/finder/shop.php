...
class ome_finder_shop{
    ...
    var $detail_jzorderdelivery = '家装订单发货';
    var $detail_jd_dly_corp = '京东物流公司配置';
    ...
    function detail_jzorderdelivery($shop_id)
    {
        $render = app::get('ome')->render();
        $oShop = app::get('ome')->model("shop");
    ...
    function detail_jd_dly_corp($shop_id)
    {
        $render = app::get('ome')->render();
        if ($_POST) {
            $jdcorp_config= array(
        ...
        $db = kernel::database();
        $dlycorps = $db->select("SELECT d.corp_id, d.branch_id, d.name, d.type FROM sdb_ome_dly_corp as d LEFT JOIN sdb_logisticsmanager_channel as c on d.channel_id=c.channel_id WHERE c.channel_type='360buy' AND d.disabled='false'");
        $render->pagedata['shop_id'] = $shop_id;
