...
class ome_finder_refund_apply{
    ...
    function detail_basic($apply_id)
    {
        ...
        if ($is_archive) {
            $oOrder = app::get('archive')->model('orders');            $order_id = $render->pagedata['refunddata']['order_id'];
...
/*暂注释 edit by sunjing 2015-4-30*/
    ...
    function column_consignee_name($row){
        ...
        if ($archive == '1' || in_array($source,array('archive'))) {
            $archive_ordObj = kernel::single('archive_interface_orders');
            $order = $archive_ordObj->getOrders(array('order_id'=>$order_id),'ship_name');
        }else{
    ...
    function column_member_name($row) {
        ...
        if ($archive == '1' || in_array($source,array('archive'))) {
            ...
            $archive_ordObj = kernel::single('archive_interface_orders');
            $order = $archive_ordObj->getOrders(array('order_id'=>$order_id),'member_id');
        }else{
            $orderObj = app::get('ome')->model('orders');
