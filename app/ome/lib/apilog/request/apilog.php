...
class ome_apilog_request_apilog extends ome_apilog_rpc{
    ...
    */
    public function getlist($filter = array(),$offset = '',$limit = '',$orderby = array()){
        ...
        if ($filter['params']) {
            $params['filter']['log_type'] = 'store.trade.stock';
            $params['filter']['params|has'] = $filter['params'];
            unset($params['filter']['params']);
            $response = $this->request('tg.api.querydetail',APILOG_URL,$params);
        }else{
