    ...
    public function process($data){
                ...
                if (isset($num[$n])){
                    ...
                    if ($oi['quantity'] != $num[$n] || $oi['price'] != $price[$n]){
                        ...
                        if ($oi['delete'] == 'true'){
                            ...
                            #[增加]货品店铺冻结 ExBOY
                            $shopFreezeLib->freeze($shop_id, $oi['product_id'], $num[$n]);
                            
                            #店铺冻结增减明细日志
                            if($is_shop_freeze_log)
                            {
                                $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $num[$n], 5, '订单号:'. $data['order_bn'] .'编辑goods时提交的数据(删除原订单上的货品并变更购买数量)');
                            }
                            ...
                            if ($t_n < 0){
                                ...
                                #[释放]货品店铺冻结 ExBOY
                                $shopFreezeLib->unfreeze($shop_id, $oi['product_id'], abs($t_n));
                                
                                #店铺冻结增减明细日志
                                if($is_shop_freeze_log)
                                {
                                    $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], abs($t_n), 6, '订单号:'. $data['order_bn'] .'编辑goods时提交的数据(货品购买数量减少)');
                                }
                                ...
                                #[增加]货品店铺冻结 ExBOY
                                $shopFreezeLib->freeze($shop_id, $oi['product_id'], abs($t_n));
                                
                                #店铺冻结增减明细日志
                                if($is_shop_freeze_log)
                                {
                                    $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], abs($t_n), 5, '订单号:'. $data['order_bn'] .'编辑goods时提交的数据(货品购买数量增加)');
                                }
                        ...
                        if($is_shop_freeze_log)
                        {
                            $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $num[$n], 5, '订单号:'. $data['order_bn'] .'编辑goods时提交的数据(删除原订单上的货品)');
                        }
                    }
                    ...
                    if ($oi['delete'] == 'false'){
                        ...
                        #[释放]货品店铺冻结 ExBOY
                        $shopFreezeLib->unfreeze($shop_id, $oi['product_id'], $oi['quantity']);
                        
                        #店铺冻结增减明细日志
                        if($is_shop_freeze_log)
                        {
                            $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $oi['quantity'], 6, '订单号:'. $data['order_bn'] .'编辑goods时提交的数(删除原订单上显示的货品)');
                        }
                    }
                    $tmp_item = array (
                        'item_id' => $n,
        ...
        if ($new_num){
            ...
            foreach ($new_num as $key => $n){
                ...
                #[增加]货品店铺冻结 ExBOY
                $shopFreezeLib->freeze($shop_id, $key, $n);
                
                #店铺冻结增减明细日志
                if($is_shop_freeze_log)
                {
                    $shopFreezeLogLib->changeLog($shop_id, $key, $n, 5, '订单号:'. $data['order_bn'] .'编辑goods时提交的数据(新增的货品)');
                }
            }
            $is_order_change = true;
            $is_goods_modify = true;
