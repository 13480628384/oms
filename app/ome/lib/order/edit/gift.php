                ...
                if (isset($num[$n])){
                    ...
                    if ($oi['quantity'] != $num[$n] || $oi['price'] != $price[$n]){
                        ...
                        if ($oi['delete'] == 'true'){
                            ...
                            {
                                $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $num[$n], 5, '订单号:'. $data['order_bn'] .'编辑Gift时提交的数据时,显示原删除的货品并变更购买数量');
                            }
                            ...
                            if ($t_n < 0){
                                ...
                                if($is_shop_freeze_log)
                                {
                                    $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], abs($t_n), 6, '订单号:'. $data['order_bn'] .'编辑Gift时提交的数据时,货品购买数量减少');
                                }
                                ...
                                if($is_shop_freeze_log)
                                {
                                    $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], abs($t_n), 5, '订单号:'. $data['order_bn'] .'编辑Gift时提交的数据时,货品购买数量增加');
                                }
                        ...
                        if($is_shop_freeze_log)
                        {
                            $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $num[$n], 5, '订单号:'. $data['order_bn'] .'编辑Gift时提交的数据时,显示原删除的货品');
                        }
                    }
                    ...
                    if ($oi['delete'] == 'false'){
                        ...
                        if($is_shop_freeze_log)
                        {
                            $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $oi['quantity'], 6, '订单号:'. $data['order_bn'] .'编辑Gift时提交的数据时,删除原显示的货品');
                        }
                    }
                    $tmp_item = array (
                        'item_id' => $n,
