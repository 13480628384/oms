            ...
            if (isset($onum[$k])){
                ...
                foreach ($v as $n){
                    ...
                    if ($oi['quantity'] != $quantity || $oi['price'] != $item_price || $oi['pmt_price'] != $pmt_price){
                        ...
                        if ($oi['delete'] == 'true'){
                            ...
                            #[增加]货品店铺冻结 ExBOY
                            $shopFreezeLib->freeze($shop_id, $oi['product_id'], $quantity);
                            
                            #店铺冻结增减明细日志
                            if($is_shop_freeze_log)
                            {
                                $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $quantity, 5, '订单号:'. $data['order_bn'] .'编辑PKG时提交的数据时,显示原删除的货品并变更购买数量');
                            }
                            ...
                            if ($t_n < 0){
                                ...
                                if($is_shop_freeze_log)
                                {
                                    $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], abs($t_n), 6, '订单号:'. $data['order_bn'] .'编辑PKG时提交的数据时,货品购买数量减少');
                                }
                                ...
                                if($is_shop_freeze_log)
                                {
                                    $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], abs($t_n), 5, '订单号:'. $data['order_bn'] .'编辑PKG时提交的数据时,货品购买数量增加');
                                }
                        ...
                        if($is_shop_freeze_log)
                        {
                            $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $quantity, 5, '订单号:'. $data['order_bn'] .'编辑PKG时提交的数据时,显示原删除的货品');
                        }
                    }
                ...
                foreach ($v as $n){
                    ...
                    if ($oi['delete'] == 'false'){
                        ...
                        if($is_shop_freeze_log)
                        {
                            $shopFreezeLogLib->changeLog($shop_id, $oi['product_id'], $oi['quantity'], 6, '订单号:'. $data['order_bn'] .'编辑PKG时提交的数据时,删除原显示的货品');
                        }
                    }
                    $tmp_obj[$k]['items'][$n]['item_id'] = $n;
        ...
        if ($newPkgONum){
            ...
            foreach ($newPkgONum as $key => $n){
                ...
                foreach ($newPkgINum[$key] as $k => $v){
                    ...
                    if($is_shop_freeze_log)
                    {
                        $shopFreezeLogLib->changeLog($shop_id, $key, $n, 5, '订单号:'. $data['order_bn'] .'编辑PKG时提交的数据时,新增的货品');
                    }
                }
