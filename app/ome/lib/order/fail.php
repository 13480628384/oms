                ...
                if($bn){
                    ...
                    if($product){
                        ...
                        if($is_normal_goods){
                            ...
                            $diff_quantity    = intval($itemInfo['quantity']) - intval($itemInfo['sendnum']);
                            $productObj->chg_product_store_freeze($item['product_id'], $diff_quantity,"+");
                            
                            #过滤失败订单上不存在的货品
                            if($item['product_id'])
                            {
                                #[增加]货品店铺冻结 ExBOY
                                $shopFreezeLib->freeze($frst_info['shop_id'], $item['product_id'], $diff_quantity);
                                
                                #店铺冻结增减明细日志
                                if($is_shop_freeze_log)
                                {
                                    $shopFreezeLogLib->changeLog($frst_info['shop_id'], $item['product_id'], $diff_quantity, 9, '失败订单号:'. $frst_info['order_bn'] .'恢复');
                                }
                            }
                        }
                    ...
                    if($product){
                        ...
                        if($is_normal_goods){
                            ...
                            if($item['product_id'])
                            {
                                ...
                                if($is_shop_freeze_log)
                                {
                                    $shopFreezeLogLib->changeLog($frst_info['shop_id'], $item['product_id'], $diff_quantity, 9, '失败订单号:'. $frst_info['order_bn'] .'恢复批量修改');
                                }
                            }
                        }
