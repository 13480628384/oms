    ...
    public function order_revoke($order_id,$reback_price='',$revock_price=''){
        ...
        if ($cancel_items){
            foreach ($cancel_items as $c_key=>$c_items){
                ...
                #[释放]货品店铺冻结 ExBOY
                $shopFreezeLib->unfreeze($order_detail['shop_id'], $c_items['product_id'], $c_items['cancel_num']);
                
                #店铺冻结增减明细日志
                if($is_shop_freeze_log)
                {
                    $shopFreezeLogLib->changeLog($order_detail['shop_id'], $c_items['product_id'], $c_items['cancel_num'], 11, '订单号:'. $order_detail['order_bn'] .',余单撤消删除未发货的货品');
                }
        ...
        if(empty($sales_row['num']))
        {
            ...
            if($delivery_id)
            {
                ...
                $orderSplitLib->add_to_sales($iostock_sales_data);
            }
        }
