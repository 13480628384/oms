    ...
    public function structure_import_data(&$mdl,$row,&$format_row=array(),&$result){
        if( $row == null ){
            ...
            if( $row['group'] == 1 ){
                ...
                $_item = array();
                if($item){
                    $products_normal_mdl = app::get('ome')->model('products_normal');
                    foreach($item as $v){
                        $inline = $v['_inline'];
                        $product = $products_normal_mdl->getlist('product_id',array('bn'=>$v['*:商品货号']),0,1);
    ...
    private function field_verify($row,&$mdl,$is_judge_repeat = false){
        ...
        if($data['item']){
            foreach($data['item'] as $itemLine => $item){
                ...
                $products = $products_mdl->getlist('product_id',array('bn'=>$item['bn']),0,1);
