    ...
    */
    ...
    public function prepared_import_csv_row($row,&$title,&$tmpl,&$mark,&$newObjFlag,&$msg)
    {
        ...
        if ($dataIdentity == 'master') {
            ...
            $sdf['verification_flag'] = kernel::single('finance_ar')->get_verification_flag($sdf['order_bn'],$sdf['money']);
            ...
            if (!isset($products[$sdf['bn']])) {
                $p = app::get('ome')->model('products')->getList('bn,product_id',array('bn'=>$sdf['bn']),0,1);
                if (empty($p)) {
    ...
    public function import_run(&$cursor_id,$params)
    {
        ...
        foreach ($params['sdfdata'] as $key => $value) {
            ...
            $this->db->beginTransaction();
            if ($value['verification_flag'] == 0) {
                $value['verification_flag'] = kernel::single('finance_ar')->get_verification_flag($value['order_bn'],$value['money']);
            }
            $result = $this->save($value);
            if ($result) {
                $this->db->commit();
