...
class wmsmgr_finder_wms{
    ...
    var $detail_invoice_issued_id= '发票相关';
    function detail_invoice_issued_id($wms_id){
        $render = app::get('wmsmgr')->render();
        if($_POST){
            $invoice_issued_id = $_POST['invoice_issued_id'];
            $wms_order_invoice = $_POST['wms_order_invoice'];
            app::get('wmsmgr')->setConf('invoice_issued_id'.$wms_id,$invoice_issued_id);
            app::get('wmsmgr')->setConf('wms_order_invoice'.$wms_id,$wms_order_invoice);
        }
        $invoice_issued_id = app::get('wmsmgr')->getConf('invoice_issued_id'.$wms_id);
        $wms_order_invoice = app::get('wmsmgr')->getConf('wms_order_invoice'.$wms_id);

         $render->pagedata['invoice_issued_id'] = $invoice_issued_id;
        $render->pagedata['wms_order_invoice'] = $wms_order_invoice;
        $render->pagedata['wms_id'] = $wms_id;
        return $render->fetch("invoiceissued.html");
    }
}