...
class sales_aftersale_type_change{
	...
	function generate_aftersale($reship_id = null){
        ...
        //获取退货单明细
        $reshipitemData = $Oreship_items->db->select("SELECT * FROM sdb_ome_reship_items WHERE reship_id=".$reship_id." AND  ((defective_num>0 OR normal_num>0) OR return_type='change') ");
