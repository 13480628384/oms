...
class wms_mdl_delivery extends dbeav_model{
    ...
    function getProductPosByDeliveryId($dly_id=null){
        ...
        $sql = "SELECT di.bn,di.product_name,di.product_id,p.barcode,
    ...
    */
    ...
    function countNoProcessDeliveryBill(){
     ...
     function fgetlist_csv( &$data,$filter,$offset,$exportType = 1 ){
       ...
       $oShop = app::get('ome')->model('shop');
        ...
        $sql = 'SELECT DI.item_id, D.delivery_bn, D.delivery_id, D.member_id, D.logi_name, D.logi_no, D.ship_addr, D.ship_area, D.ship_name, D.ship_tel, D.ship_mobile, D.delivery, D.delivery_time, D.ship_zip, DI.bn, product.name product_name, DI.number as dn
    ...
    function getDeliveryByOrder($order_id)
    {
        ...
        foreach($rows as $key => $row){
            ...
            $delivery = $this->getlist('delivery_bn,create_time,status,ship_name,branch_id,delivery_id,print_status,process_status,logi_name,status',array('outer_delivery_bn'=>$wms_delivery[1]));
    ...
    function getProcutInfo($delivery_id){
        $sql = 'select
	                items.bn,items.product_name,items.number,delivery.net_weight
    ...
    public function getExportDataByCustom($fields, $filter, $has_detail, $curr_sheet, $start, $end){
        ...
        $sql = 'SELECT delivery_bn,outer_delivery_bn,delivery_id FROM  sdb_wms_delivery where delivery_id in ("'.implode('","',$ids).'") ORDER BY delivery_id DESC';
        $list = $this->db->select($sql);
        $outer_delivery = array();
        ...
        foreach($rows as $key=>$row){
            ...
            $rows[$key]['delivery_bn'] = $outer_delivery[$row['delivery_bn']]."\t";
        }
