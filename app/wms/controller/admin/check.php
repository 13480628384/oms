...
class wms_ctl_admin_check extends desktop_controller{
    ...
    function saveBatchCheck(){
        ...
        // 添加日志 begin
        $deliveries = array();
        $delivery_bill_obj = kernel::single('wms_delivery_bill');
        foreach($delivery_ids as $k => $v){
            $deliveries[] = $delivery_bill_obj->getDeliveryIdByPrimaryLogi($v);
        }
        $op_obj = app::get('ome')->model('operation_log');
        $op_obj->batch_write_log('delivery_check@wms', '成功添加到批量校检队列', time(), array('delivery_id' => $deliveries));
        // end
