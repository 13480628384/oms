...
class wms_delivery_template{
    ...
    public function processElementContent($value_list)
    {
        ...
        //获取云栈大头笔信息
        $corpType = app::get('ome')->model('dly_corp')->dump(array('corp_id'=>$value_list['logi_id'],'tmpl'=>'normal'),'tmpl_type,channel_id,type');
        ...
        if ($mainoInfo) {
            ...
            $data['seller_id'] = $channelExtendInfo['seller_id'];
            $data['print_config'] = $mainoInfo['print_config'];
            $data['cp_code'] = strtoupper($corpType['type']);
        }
        ...
        $channel = $channelObj->dump(array('channel_id'=>$corpType['channel_id']),'logistics_code');

        $logistics_code = logisticsmanager_waybill_sf::logistics($channel['logistics_code']);
