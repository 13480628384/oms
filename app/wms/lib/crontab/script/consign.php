 ...
 */
class wms_crontab_script_consign
{
    ...
    public function get_logi_no(&$log_id='',&$logiNoList='')
    {
        ...
        $rows = $batchLogModel->getList('*',array('log_type'=>'consign','status'=>'0'),0,-1,'createtime asc');
    ...
    public function error($log_id,$logi_no,$msg,$failNum)
    {
        ...
        // 从数组中剔除
        $key = array_search($logi_no,$this->_log_text);
        if ($key !== false) {
    ...
    public function process($log_id,$logiNoList,$loginfo = array())
    {
        ...
        foreach ($logiNoList as $logi_no) {
            ...
            if($primary){
                ...
                $area_id = $arrArea[2];
                $delivery_cost_actual = $wmsCommonLib->getDeliveryFreight($area_id,$dly['logi_id'],$minWeight);
            }
