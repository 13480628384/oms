...
class omedlyexport_mdl_ome_delivery extends ome_mdl_delivery{
    ...
    public function getExportDataByCustom($fields, $filter, $has_detail, $curr_sheet, $start, $end){
        ...
        // 批次
        // $f_queue_items = array();
        // $queue_items = $obj_queue_items->getlist('ident,ident_dly',array('delivery_id'=>$ids));
        // foreach ($queue_items as $value) {
        //     $f_queue_items[$value['delivery_id']] = $value;
        // }
        // unset($queue_items);
        ...
        // 发货单
        $sql = sprintf('SELECT branch_id, delivery_bn, delivery_id, member_id, logi_name, logi_no, ship_addr, ship_area, ship_name, ship_tel, ship_mobile, delivery_time, ship_zip,create_time FROM sdb_ome_delivery WHERE delivery_id in(%s)',implode(',',$ids));
        ...
        $sql = sprintf('SELECT order_id, order_bn, custom_mark, mark_text, shop_id, tax_no, cost_freight FROM sdb_ome_orders where order_id in(%s)',implode(',',$orderIds));
        $list = $this->db->select($sql);
        foreach ($list as $key => $value) {
        ...
        // 货品
        $sql = sprintf('SELECT product_id,name AS product_name,spec_desc FROM sdb_ome_products WHERE product_id in(%s)',implode(',',$productIds));
        $list = $this->db->select($sql);
        foreach ($list as $key => $value) {
        ...
        // $sql = 'SELECT DI.item_id, D.branch_id, D.delivery_bn, D.delivery_id, D.member_id, D.logi_name, D.logi_no, D.ship_addr, D.ship_area, D.ship_name, D.ship_tel, D.ship_mobile, D.delivery_time, D.ship_zip, DI.bn, product.name as product_name, DI.number as dn,D.create_time,product.spec_desc
        //     FROM sdb_ome_delivery_items AS DI
        //     LEFT JOIN sdb_ome_delivery AS D  ON D.delivery_id = DI.delivery_id 
        //     LEFT JOIN sdb_ome_products as product on DI.product_id=product.product_id
        //     where D.delivery_id in ('.implode(',',$ids).') ORDER BY D.delivery_id DESC';
        ...
        // //[拆单]获取多个发货单对应订单信息 ExBOY
        // $sql    = "SELECT DI.delivery_id,O.order_id, O.order_bn, O.custom_mark, O.mark_text, O.shop_id, O.tax_no, O.cost_freight, 
        //             DI.bn, DI.price, DI.amount, DI.number, DI.product_id 
        ...
        foreach ($rows as &$row){
            ...
            $row['total'] = $row['cost_freight'] + $row['amount'];
            ...
            $row['spec_value']  = $spec_desc ? implode('|',$spec_desc['spec_value']) : '';
