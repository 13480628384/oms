...
//
//        $items = $this->app->model('appropriation_items')->select()->columns('*')->where('appropriation_id=?',$appropriation_id)->instance()->fetch_all();
...
//        $button = <<<EOF
//        &nbsp;&nbsp;<a href="index.php?app=taoguanallocate&ctl=admin_appropriation&act=printAppropriation&p[0]=$id" target="_bank" class="lnk">打印</a> &nbsp;&nbsp;
//EOF;
//if($appropriation_type!='directly' && $iostockorder['confirm']=='N'){
////当调拔出入库时 入库单未入库
//
//        $button.= <<<EOF
//        &nbsp;&nbsp;<a href="index.php?app=taoguanallocate&ctl=admin_appropriation&act=deleteAppropriation&p[0]=$id&finder_id=$finder_id" class="lnk">删除</a> &nbsp;&nbsp;
//EOF;
//}
//        return $button;
//    }
