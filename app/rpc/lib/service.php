...
class rpc_service{
    ...
    */
    ...
    private function sign_check(&$post,$query_params=array()){
        ...
        switch($adapter){
            case 'matrixwms':#矩阵开放平台WMS
                $app_id = $post['app_id'] ? $post['app_id'] : $query_params['app_id'];
                if ($app_id){
                ...
                if (!base_shopnode::token($app_id))
                    $sign_check = base_certificate::gen_sign($post);
                else
                    $sign_check = base_shopnode::gen_sign($post,$app_id);
                break;
            default:#其它直连WMS
                $signObj = kernel::single('rpc_sign');
                $sign_key = $this->func_instance->getSignKey(self::$node_id);
               
                $sign_check = $signObj->get_response_sign($post,$sign_key);
        }
