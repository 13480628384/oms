...
phpcgiprocess=$(ps -ef |grep php|grep taskmgr|grep -v timer|grep -v init|grep -v export|grep -v rpc|grep task)
...
phpcgiprocess=$(ps -ef |grep taskmgr|grep taskDaemon|grep rpc)

if [ -z "$phpcgiprocess" ];then
        echo "$(date)  rpc  downd  ">> $BINBASEDIR/logs/phpdown.log
        rm -rf  /var/run/erp-taskDaemon-rpc.pid
        $PHPBIN  $BINBASEDIR/taskDaemon.php start rpc >> $BINBASEDIR/logs/phpdown.log
        echo "$(date) rpc ok" >> $BINBASEDIR/logs/phpdown.log

else
        echo "$(date) rpc ok" >> /tmp/erp-chechphp.log
fi