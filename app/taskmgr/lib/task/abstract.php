...
class taskmgr_task_abstract extends Stackable{
    ...
    public function doTask($message, $queue = null){
        ...
        if($body){
            ...
            $flag = isset($content['data'][$this->_process_id]) ? $content['data'][$this->_process_id] : 'no id';

            $info = sprintf("%s\t%s\t%s\t%s bytes\t%s", $domain, $flag, $response['code'], $response['body'], $e-$t);
     ...
     */
     protected function requeue($params){
        if($this->_retry === false){
            return ;
        }

        //验签生成，数据压缩
        unset($params['data']['taskmgr_sign']);
