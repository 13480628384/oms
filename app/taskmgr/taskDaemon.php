...
class taskDaemon {
    ...
    private function start(){
		...
		switch ($this->type) {
			...
			default:
                ...
                $tasks_5 = $this->startRpc();
				$tasks = array_merge($tasks_1,$tasks_2,$tasks_3,$tasks_4,$tasks_5);
				break;
		}
    ...
    private function startTask(){
        //一般业务处理任务
        foreach(taskmgr_whitelist::task_list() as $_task => $param) {
            $obj = new taskmgr_controller_task($_task, $param['threadNum']);
            $obj->start();
            $tasks[$_task] = $obj;
        }
    ...
    private function startTimer(){
        //定时任务
        foreach(taskmgr_whitelist::timer_list() as $_task => $param) {

            $obj = new taskmgr_controller_timer($_task, $param['threadNum']);
            $obj->start();
            $tasks[$_task] = $obj;
        }
    ...
    private function startInit(){
        //初始化任务
        foreach(taskmgr_whitelist::init_list() as $_task => $param) {

            $obj = new taskmgr_controller_init($_task, $param['threadNum']);
            $obj->start();
            $tasks[$_task] = $obj;
        }
    ...
    private function startExport(){
        //导出任务
        foreach(taskmgr_whitelist::export_list() as $_task => $param) {
            
            $obj = new taskmgr_controller_export($_task, $param['threadNum']);
            $obj->start();
            $tasks[$_task] = $obj;
        }
    ...
    private function startRpc(){
        //rpc请求任务
        foreach(taskmgr_whitelist::rpc_list() as $_task => $param) {

            $obj = new taskmgr_controller_rpc($_task, $param['threadNum']);
            $obj->start();
            $tasks[$_task] = $obj;
        }
        $this->fixvirtualbtbug('taskmgr_controller_rpc', $tasks);
        return $tasks;
    }
