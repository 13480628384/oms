    ...
    */
    protected function aftersale_add(){
        ...
        if ($status == '1' || !$tgReturn['return_id']) {
            ...
            $delisql    = "SELECT dord.delivery_id FROM sdb_ome_delivery_order AS dord LEFT JOIN sdb_ome_delivery AS d
            ...
            $deliOrder = kernel::database()->selectrow($delisql);
...
//            }
    ...
    */
    ...
    protected function _updateLogistic($reship_id,$company_name,$sid){
        ...
        $Oreship = app::get(self::_APP_NAME)->model('reship');
        $oOperation_log = app::get(self::_APP_NAME)->model('operation_log');
        $dlyCorpObj = app::get(self::_APP_NAME)->model('dly_corp');
        ...
        if ($company_name && $sid) {
            ...
            if ($reship) {
                $dlyCorpInfo = $dlyCorpObj->dump(array('name'=>$company_name),'corp_id');
                $updata = array(
                        'return_logi_name'=>$dlyCorpInfo['corp_id'],
                        'return_logi_no'=>$sid,
                        'outer_lastmodify'=>$this->_refundsdf['modified'],
                );
