...
abstract class apibusiness_response_order_360buy_abstract extends apibusiness_response_order_abstractbase
{
    ...
    protected function canUpdate()
    {
        ...
        #6='退款申请中',7='退款中',
        $pay_status = array(6,7);
        #货到付款订单，属于退款申请中的，如果再更新，会导致支付状态变为已支付
        if($this->_ordersdf['shipping']['is_cod'] == 'true'){
            if (in_array($this->_tgOrder['pay_status'], $pay_status)) {
                $this->_apiLog['info']['msg'] = '订单有退款，不接受!';
                return false;
            }
        }
        return $rs;
    ...
    public function canCreate()
    {
        ...
        if ($this->_ordersdf['pay_status'] == '7') {
            $this->_apiLog['info']['msg'] = '锁定的订单不接收';
            return false;
        }     
