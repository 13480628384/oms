...
*/
class apibusiness_response_order_component_items360buy extends apibusiness_response_order_component_abstract
{
    const _APP_NAME = 'ome';
    ...
    public function update()
    {
        ...
        if ($this->_platform->_tgOrder['ship_status'] == '0') {
            ...
            foreach ((array)$ordersdf['order_objects'] as $object) {
                ...
                $tmpobjbn = $object['bn'] ? $object['bn'] : uniqid();
                $object['obj_type'] = $object['obj_type'] ? $object['obj_type'] : 'goods';

                $objkey = sprintf('%u',crc32(trim($tmpobjbn) . '-' . trim($object['obj_type'])));
                ...
                $object_pmt = 0; $order_items = array();
                foreach ((array)$object['order_items'] as $item) {
                    $tmpitembn = $item['bn'] ? $item['bn'] : uniqid();
                    $item['item_type'] = $item['item_type'] ? $item['item_type'] : 'product';
                    
                    $itemkey = sprintf('%u',crc32(trim($tmpitembn) . '-' . trim($item['item_type'])));
