...
* @package apibusiness/response/order/component/items
* @author chenping<chenping@shopex.cn>
* @version $Id: items.php 2013-3-12 17:23Z
*/
class apibusiness_response_order_component_shopexitems extends apibusiness_response_order_component_abstract
{
    const _APP_NAME = 'ome';
    ...
    public function convert()
    {
        foreach ($this->_platform->_ordersdf['order_objects'] as $object) {
            ...
            foreach ($object['order_items'] as $item) {
                ...
                $item_sale_price = (isset($item['sale_price']) && is_numeric($item['sale_price']) && -1 != bccomp($item['sale_price'], 0, 3) ) ? $item['sale_price'] : bcsub($subtotal, (float) $item['pmt_price'],3);
                $order_items[] = array(
                    ...
                    'sale_price'      => (float) $item_sale_price,
                    'amount'          => $subtotal,
    ...
    public function update()
    {
        ...
        if ($this->_platform->_tgOrder['ship_status'] == '0') {
            ...
            foreach ((array)$ordersdf['order_objects'] as $object) {
                ...
                $obj_sale_price = (isset($object['sale_price']) && is_numeric($object['sale_price']) && -1 != bccomp($object['sale_price'], 0, 3) ) ? (float)$object['sale_price'] :  bcsub($obj_amount,bcadd((float)$object['pmt_price'], $object_pmt,3),3);
                ...
                foreach ($object['order_items'] as $itemkey=>$item) {
                    ...
                    if (!$ordersdf_object[$objkey]['order_items'][$itemkey]) {
                        ...
                        if ($item['product_id']) {
                            ...
                            #[释放]货品店铺冻结 ExBOY
                            $shopFreezeLib->unfreeze($this->_platform->_tgOrder['shop_id'], $item['product_id'], $item['quantity']);
                            
                            #店铺冻结增减明细日志
                            if($is_shop_freeze_log)
                            {
                                $shopFreezeLogLib->changeLog($this->_platform->_tgOrder['shop_id'], $item['product_id'], $item['quantity'], 3, '更新订单号:'. $this->_platform->_tgOrder['order_bn'] .'明细,扣冻结');
                            }
                        }
                    }
                }
                ...
                foreach ($order_items as $itemkey => $item) {
                    ...
                    if ($diff_item) {
                        ...
                        if ($diff_item['delete'] == 'false' && $item['product_id']) {
                            ...
                            #[增加]货品店铺冻结 ExBOY
                            $shopFreezeLib->freeze($this->_platform->_tgOrder['shop_id'], $item['product_id'], $item['quantity']);
                            
                            #店铺冻结增减明细日志
                            if($is_shop_freeze_log)
                            {
                                $shopFreezeLogLib->changeLog($this->_platform->_tgOrder['shop_id'], $item['product_id'], $item['quantity'], 2, '更新订单号:'. $this->_platform->_tgOrder['order_bn'] .'明细,增加冻结');
                            }
                            ...
                            if($is_shop_freeze_log)
                            {
                                $shopFreezeLogLib->changeLog($this->_platform->_tgOrder['shop_id'], $item['product_id'], $item['quantity'], 3, '更新订单号:'. $this->_platform->_tgOrder['order_bn'] .'明细,释放冻结');
                            }
                            ...
                            {
                                #释放
                                $shopFreezeLib->unfreeze($item['product_id'], $this->_platform->_tgOrder['shop_id'], $item['quantity']);
