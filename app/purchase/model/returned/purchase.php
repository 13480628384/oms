...
class purchase_mdl_returned_purchase extends dbeav_model{
   ...
   function modifier_arrive_time($row){
    ...
    function fgetlist_csv( &$data,$filter,$offset,$exportType = 1){
        ...
        if(!$data['title']){
            $title = array();
            foreach($this->io_title('return') as $ks => $vs){
                $title[] = $vs;
            }
        ...
        foreach($list as $key => $value){
            $supplier = app::get('purchase')->model('supplier')->dump($value['supplier_id']);
            $branch = app::get('ome')->model('branch')->dump($value['branch_id']);

            $contents = '';
            foreach(unserialize($value['memo']) as $kk => $vv){
                $contents .= $vv['op_content'].' '.$vv['op_time'].' by '.$vv['op_name'].'; ';
            }

            $row_val = array();
            $row_val['*:退货单号(CSCID)'] = $value['rp_bn'];
            $row_val['*:退货单名称'] = $value['name'];
            $row_val['*:供应商'] = $supplier['name'];
            $row_val['*:经办人'] = $value['operator'];
            $row_val['*:退货日期'] = date('Y-m-d', $value['returned_time']);
            $row_val['*:采购日期'] = date('Y-m-d', $value['purchase_time']);
            $row_val['*:仓库'] = $branch['name'];
            $row_val['*:金额总计'] = $value['amount'];
            $row_val['*:商品总额'] = $value['product_cost'];
            $row_val['*:物流费用'] = $value['delivery_cost'];
            $row_val['*:物流单号'] = $value['logi_no'];
            $row_val['*:采购入库单ID'] = $value['object_id'];
            $row_val['*:是否特别退货'] = $value['emergency'] == 'true'? '是': '否';
            $row_val['*:物流公司ID'] = $value['corp_id'];
            $row_val['*:备注'] = $contents;

            $data['content'][] = '"' . implode('","', $row_val) . '"';
        }
