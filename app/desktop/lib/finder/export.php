...
class desktop_finder_export{
    ...
    private function main(){
        ...
        //导出主明细数据
        $export_data['content']['main'] = $this->item_list_body($list, $colArray, $key_modifier, $object_modifier, $type_modifier);
     ...
     * @return null
     */
    function &item_list_body(&$list, &$colArray, &$key_modifier, &$object_modifier, &$type_modifier) {
        $body = array();
        $curr_row = 1;
        foreach ($list as $i => $row) {
            foreach ((array) $colArray as $k => $col) {
                ...
                if ($col['type'] == 'func') {
                    ...
                    $body[$curr_row][] = $a = $col['ref'][0]->{$col['ref'][1]}($row);
                } elseif (isset($key_modifier[$k])) {
                    $this->object->pkvalue = $row[$this->dbschema['idColumn']];
                    $body[$curr_row][] = $this->object->{$key_modifier[$k]}($row[$k]);
