    ...
    function _actions() {
        ...
        if ($this->use_buildin_export) {
            if(isset($this->object->has_export_cnf) && $this->object->has_export_cnf){
                ...
                $actions[] = array('label' => app::get('desktop')->_('导出'), 'icon' => 'download.gif', 'submit' => $this->url . '&action=export&finder_aliasname='.$this->finder_aliasname, 'target' => 'dialog::{width:400,height:170,title:\'' . app::get('desktop')->_('导出') . '\'}');
            }
        }
        ...
        if (isset($actions) && $actions) {
            foreach ($actions as $key => $item) {
                ...
                if ($item['href']) {
                    $item['href'] = $item['href'] . '&_finder[finder_id]=' . $finder_name . '&finder_id=' . $finder_name;
                } else {
...
EOF;
    ...
    {
        $this->_tab_view_data = parent::get_views();
