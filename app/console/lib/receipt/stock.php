...
//
        ...
        if ($io == '0' && !$data['items'] && $io_update_data['iso_status'] == '3'){
    ...
    */
    ...
    private function addArriveStore($data)
    {
        ...
        foreach ($items as $item) {
            if ($item['nums'] > 0) {
                //如果该商品没有该仓库的库存 如果不存在库存数据，则插入一条（包含单位成本、库存成本）
                $data_branch_product = $ome_branch_product_obj->dump(array('product_id'=>$item['product_id'],'branch_id'=>$to_branch_id),'branch_id');
                if(!$data_branch_product){
                    $branch_product_insert_new = array(
                    ...
                    $ome_branch_product_obj->save($branch_product_insert_new);
                }
    ...
    */
    public function clear_stockout_store_freeze($data,$io_type='-'){
        ...
        if ($data['items']){
            ...
            $items = $oItems->getlist('product_id,bn,nums,normal_num',array('iso_id'=>$iso['iso_id']),0,-1);
        }
