...
class console_finder_delivery{
    ...
    var $addon_cols = "skuNum,itemNum,bnsContent,delivery_id,status,process,stock_status,deliv_status,expre_status,verify,is_bind,type,bind_key,order_createtime,deli_cfg,is_cod,logi_number,delivery_logi_number,logi_no";
    ...
    #买家留言
     function column_custom_mark($row){
         #根据物流单号，获取买家留言
         $markInfo    = $this->get_cache_mark($row['delivery_id']);
...
//    }
     ...
     * 获取买家留言 与 客服备注 ExBOY
     */
    function get_cache_mark($delivery_id)
    {
        if(empty($this->cache_mark[$delivery_id]))
        {
            $dlyorderObj    = app::get('ome')->model('delivery_order');
            
            $markInfo       = $dlyorderObj->getOrderInfo('custom_mark, mark_text', $delivery_id);
            
            #缓存
            $this->cache_mark[$delivery_id]    = $markInfo;
            unset($markInfo);
        }
        
        return $this->cache_mark[$delivery_id];
    }
}
?>