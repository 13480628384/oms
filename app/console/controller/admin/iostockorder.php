...
class console_ctl_admin_iostockorder extends desktop_controller{
    ...
    function iostock_add($type,$io){
        ...
        //出入库计划-入库单新建：移除type_id 11 调拨入库取消 作为选择项 此出入库类型为取消调拨入库时系统生成
    ...
    */
    function cancel($iso_id,$io,$type){
        ...
        unset($iso);
        if($iso_type_id == "4"){
            $this->display("admin/iostock/stockin_cancel.html");
        }else{
    ...
    */
	...
	function cancelIostockin(){
        $iso_id = intval($_GET['iso_id']);
        $this->pagedata['cur_date'] = date('Ymd',time())."入库单";
        $operator = kernel::single('desktop_user')->get_name();
        $this->pagedata['operator'] = $operator;
	    ...
	    $rs_products = $productObj->getList("product_id,spec_info,barcode,price",array("product_id|in"=>$arr_products_ids));
	...
	//生成入库单 打回原始仓库
	function doCancelIostockin(){
	    $this->begin('');
	    $type_id = 11; //调拨入库取消 type_id
        $iso_id = intval($_POST["iso_id"]);           	    
        $taoguaniostockorder_iso_obj = app::get('taoguaniostockorder')->model('iso');
        $taoguaniostockorder_iso_items_obj = app::get('taoguaniostockorder')->model('iso_items');
        $info_iso =  $taoguaniostockorder_iso_obj->dump(array('iso_id'=>$iso_id),"*");
        $info_iso_items = $taoguaniostockorder_iso_items_obj->getList("*",array('iso_id'=>$iso_id));
        ...
        //新增调拨入库取消的入库单
        $insert_iostockin_arr = $info_iso;
        unset($insert_iostockin_arr["iso_id"]);
        $insert_iostockin_arr["memo"] = htmlspecialchars($_POST['memo']);
        $insert_iostockin_arr['oper'] = $_POST['operator'];
        $insert_iostockin_arr['operator'] = $_POST['operator'];
        $insert_iostockin_arr['iso_status'] = 1;
        $insert_iostockin_arr['check_status'] = 2;//生成打回入库单同时审核成功
        $insert_iostockin_arr['type_id'] = $type_id;
        $insert_iostockin_arr["name"] = $_POST["iostockorder_name"];
        $insert_iostockin_arr["iso_bn"] = kernel::single('console_iostockorder')->get_iostockorder_bn($type_id);
        $insert_iostockin_arr["original_bn"] = $info_iso["iso_bn"];
        $insert_iostockin_arr["original_id"] = $info_iso["iso_id"];
        $insert_iostockin_arr["create_time"] = time();
        //打回库存：目的仓和原仓互换
        $insert_iostockin_arr["branch_id"] = $info_iso["extrabranch_id"];
        $insert_iostockin_arr["extrabranch_id"] = $info_iso["branch_id"];
        $taoguaniostockorder_iso_obj->save($insert_iostockin_arr);
        $iso_id = $taoguaniostockorder_iso_obj->db->lastInsertId();

        //新增入库表明细 并处理商品在途库存 和 总库存
        foreach ($info_iso_items as $var_iso_item){
            $temp_item_insert_arr = $var_iso_item;
            unset($temp_item_insert_arr["iso_items_id"]);
            $temp_item_insert_arr["iso_id"] = $iso_id;
            $temp_item_insert_arr["iso_bn"] = $insert_iostockin_arr["iso_bn"];
            $taoguaniostockorder_iso_items_obj->save($temp_item_insert_arr);
            $this->clear_arrive_store_back_to_store(intval($temp_item_insert_arr["product_id"]),intval($temp_item_insert_arr["nums"]),intval($info_iso["branch_id"]),intval($info_iso["extrabranch_id"]));
        }
        
        //更新原入库单为取消状态 iso_status 4
        $iso_status_update = array('iso_status'=>'4');
        $taoguaniostockorder_iso_obj->update($iso_status_update,array('iso_id'=>$info_iso["iso_id"]));
        
        //事件触发，通知oms出入库
        kernel::single('wms_event_trigger_otherinstorage')->inStorage(array('iso_id'=>$iso_id), true);
        
	    $this->end(true,"已确认取消入库");
	}
	
    function clear_arrive_store_back_to_store($product_id,$nums,$cancel_branch_id,$back_branch_id){
        if(!$product_id || !$nums || !$cancel_branch_id || !$back_branch_id){
            return;
        }
        $nums = intval($nums);
        $time = time();
        $ome_branch_product_obj = app::get('ome')->model('branch_product');
        $arrive_store_info = $ome_branch_product_obj->dump(array('product_id'=>$product_id,'branch_id'=>$cancel_branch_id),"arrive_store");
        if ($nums > 0) {
            //cancel arrive_store
            $sql = "UPDATE sdb_ome_branch_product SET arrive_store = IF(arrive_store>={$nums},arrive_store-{$nums},0) ,last_modified = {$time} WHERE product_id = {$product_id} AND branch_id={$cancel_branch_id}";
            $ome_branch_product_obj->db->exec($sql);
            //back to store not use now
//             $sql = "UPDATE sdb_ome_branch_product SET store = store+{$nums} ,last_modified = {$time} WHERE product_id = {$product_id} AND branch_id={$back_branch_id}";
//             $ome_branch_product_obj->db->exec($sql);
        }
    }
