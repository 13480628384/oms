...
class console_ctl_admin_purchase extends desktop_controller{
    ...
    public function do_check() {
        ...
        app::get('purchase')->model('po')->update(array(
            'check_status' => 2,
            'eo_status' => 1,
            'check_time'=> time(),
        ...
        //事件触发，通知wms创建采购通知单 add by danny event notify
        #新增在途库存

        kernel::single('console_event_trigger_purchase')->create(array('po_id'=>$_POST['po_id']), false);
        #kernel::single('console_iostockdata')->notify_purchase($_POST['po_id'],$aRow['branch_id']);
    ...
    function findProduct($supplier_id=null){
        ...
        if ($supplier_id){
            ...
            $products = $oProduct->getList('product_id', $supplier_goods, 0, -1);
            $product_list = array();
            if ($products)
            foreach ($products as $k=>$v){
                $product_list[] = $v['product_id'];
            }
            $base_filter = array('product_id'=>$product_list);
        }
        ...
        if (!empty($data)){
            foreach ($data as $k => $item){
                ...
                if($supplier_id > 0){
                    ...
                    $product = $pObj->dump(array('product_id'=>$item['product_id']),'cost');

                    $item['price'] = $product['price']['cost']['price'];
                }
    ...
    function getEditProducts($po_id){
        ...
        if ($items){
            ...
            if($product_ids){
                $plist = $pObj->getList('product_id,visibility',array('product_id'=>$product_ids));
    ...
    function doSave(){
        ...
        $pObj = app::get('ome')->model('products');
        if ($ids)
        foreach ($ids as $i){//插入采购单详情
            $p = $pObj->dump($i, 'barcode,bn,name,spec_info');
            $row['nums'] = $at[$i];
            $row['price'] = $pr[$i];
            $row['bn'] = $p['bn'];
            $row['name'] = $p['name'];
    ...
    function printItem($po_id,$type='po'){
        ...
        foreach($poo['po_items'] as $key=>$product_items){
            $goodsInfo = array();
           $last_product_info = $prObj->dump($product_items['product_id'],'goods_id,bn,name,spec_info');

           //根据货品查找相关商品的信息
           if($last_product_info){
                $goodsInfo = $goodsObj->getList('bn,unit',array('goods_id'=>$last_product_info['goods_id']),0,1);
           }
           ...
           #检测货号是否变化
           if(strcasecmp($product_items['bn'],$last_product_info['bn']) != 0){
           ...
           #检测商品名称是否变化
           if(strcasecmp($product_items['name'],$last_product_info['name'])!=0){
               $product_items['name'] = $product_items['name'].'('.$last_product_info['name'].')';
           }
    ...
    function addSame($po_id){
        ...
        $brObj = app::get('ome')->model('branch');
        $row = $brObj->getList('branch_id, name',array('type'=>'main'),0,-1);
    ...
    *
    */
    function purchaseConfirm($po_id) {
        $poObj = app::get('purchase')->model('po');
        $eoObj = app::get('purchase')->model('eo');
        $suObj = app::get('purchase')->model('supplier');
        $brObj = app::get('ome')->model('branch');
    ...
    */
    function doPurchaseconfirm(){
        ...
        if( empty($damagedbranch) ){
            ...
            $this->end(false,$items['bn'].'有不良品，但未设置主仓对应的残仓');
        }
    ...
    */
    public function checkCancel($po_id){
        $poObj = app::get('purchase')->model('po');
        $po = $poObj->dump($po_id, '*', array('po_items'=>array('*')));
        if ($po['check_status'] == '2'){#已审核，需要请求第三方
    ...
    function batch_sync()
    {
        ...
        $po_list = $poObj->getList('po_id',array('check_status'=>array('2'),'po_status'=>array('1'),'eo_status'=>array('0','1')));
