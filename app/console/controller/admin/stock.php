...
class console_ctl_admin_stock extends desktop_controller{
    ...
    function index(){
       ...
       $this->finder('console_mdl_products',array(
            'title'=>'总库存列表',
            'base_filter' => $filter,
            'actions' => $actions,
    ...
    function search(){
        if($_POST['stock_search']){
            ...
            $data = app::get('ome')->model('products')->search_stockinfo($keywords);
    ...
    function show_store_freeze_list(){
        ...
        foreach($store_freeze as $k=>$v){
            ...
            $rows['process_status']   = $v['process_status'];
            $rows['process_title']    = $statusObj->process_status($v['process_status']);
        ...
        $stock_sql = "select ai.product_id,a.branch_id,ai.nums,a.iso_bn from sdb_taoguaniostockorder_iso as a LEFT JOIN sdb_taoguaniostockorder_iso_items as ai ON a.iso_id = ai.iso_id
                        where a.iso_status in ('1','2') AND a.check_status in ('2') AND a.type_id in('5','7','100','300') AND ai.product_id in (".$product_id.")";
        $stock_list = $db->select($stock_sql);
        //换货出库
        $reship_sql = "SELECT ai.product_id,r.changebranch_id as branch_id,ai.num,r.reship_bn FROM sdb_ome_reship as r LEFT JOIN sdb_ome_reship_items as ai ON r.reship_id=ai.reship_id WHERE r.return_type='change' AND ai.return_type='change' AND r.is_check in('1','11') AND ai.product_id in (".$product_id.")";
