<?php
class siso_receipt_iostock_reshipdamaged extends siso_receipt_iostock_abstract implements siso_receipt_iostock_interface{
    /**
     *
     * 出入库类型id
     * @var int
     */
    protected $_typeId = 50;
    ...
    function get_io_data($data){
        ...
        $reship_items = $oReship_items->getlist('*',array('reship_id'=>$reship_id,'return_type'=>array('return','refuse'),'defective_num|than'=>0),0,-1);
        $operator       = kernel::single('desktop_user')->get_name();
        $operator = $operator=='' ? 'system' : $operator;
        
        $damagedbranch = kernel::single('console_iostockdata')->getDamagedbranch($reship_items[0]['branch_id']);
        if ($reship_items) {
