...
abstract class siso_receipt_iostock_abstract {
    ...
    protected $_iostock_types = array(
       ...
       '600'=>array('code'=>'V','info'=>'转储入库','io'=>1),
        ...
        '11'=>array('code'=>'X','info'=>'调拨入库取消','io'=>1),
        '700'=>array('code'=>'Z','info'=>'分销出库','io'=>0),
        '800'=>array('code'=>'Y','info'=>'分销入库','io'=>1),
    );
    ...
    private function convertSdf($data){
        ...
        foreach($data as $key=>$v){
            ...
            if($this->_io_type){ //入库
                $rs = $this->updateBranchProduct($v['nums'],$v['product_id'],$v['branch_id'],'+');
                if ($rs == false){
                    return false;
                }
