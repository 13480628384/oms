<?php
class siso_receipt_iostock_purchase extends siso_receipt_iostock_abstract implements siso_receipt_iostock_interface{
    ...
    function get_io_data($data){
        ...
        if ($data['io_items']){
            $iso_items_detail = $data['io_items'];
        }else{
        ...
        if ($iso_items_detail){
            foreach ($iso_items_detail as $k=>$v){
                $iostock_data[$v['iso_items_id']] = array(
                    ...
                    'original_bn' => $iso_detail['original_bn'],
                    'original_id' => $iso_detail['original_id'],
