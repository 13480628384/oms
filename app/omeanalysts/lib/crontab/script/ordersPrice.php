...
class omeanalysts_crontab_script_ordersPrice{
	...
	public function orderPrice(){
		...
		$sql_group_by = ' group by shop_id';
		foreach($times as $time){
			foreach($info as $key => $value){
				$sql_where_filter = array();
				$sql_where_filter[] = 'createtime >= '.$time['from_time'];
				$sql_where_filter[] = 'createtime < '.$time['to_time'];
				if(!empty($value['from'])){
					$sql_where_filter[] = 'total_amount >= '.$value['from'];
				}
				if(!empty($value['to'])){
					$sql_where_filter[] = 'total_amount < '.$value['to'];
				}
				$sql_count = 'select count(order_id) as num,shop_id,createtime FROM sdb_ome_orders WHERE '.implode(" AND ",$sql_where_filter).$sql_group_by;
				$datas = $db->select($sql_count);
				if(!empty($datas)){
	...
	/*
	//old function/already has new function
//     public function orderPrice(){
        @set_time_limit(0);
        $db = kernel::database();
