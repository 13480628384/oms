...
class omeanalysts_mdl_ome_goodsale extends dbeav_model{
    ...
    public function _get_count($filter=null){
        $sql = "select count(*) as _count from sdb_ome_sales_items as saleitem left join
                sdb_ome_sales as sales on saleitem.sale_id = sales.sale_id left join sdb_ome_products p ON saleitem.product_id = p.product_id left join sdb_ome_goods as g on p.goods_id=g.goods_id
    ...
    public function getlist($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
         ...
         $datas = array();
         $sql = "select saleitem.name,saleitem.spec_name as goods_specinfo,ords.payment,sales.branch_id,sales.shop_id,sales.order_id,sales.sale_id as sale_id,sales.iostock_bn,saleitem.bn as product_bn,saleitem.nums as buycount,saleitem.sales_amount as sale_amount,sales.sale_time as createtime
                from sdb_ome_sales_items as saleitem left join sdb_ome_sales as sales on saleitem.sale_id = sales.sale_id
                left join sdb_ome_products p ON saleitem.product_id = p.product_id
                left join sdb_ome_goods as g on p.goods_id=g.goods_id
                left join sdb_ome_orders as ords on sales.order_id=ords.order_id
                where 1 " .$this->_filter($filter);

         base_kvstore::instance('omeanalysts_goodsale')->store('goodsale_filter',serialize($filter));
         ...
         if($rows){
            foreach($rows as $row){
                ...
                $sql1 = "select product.goods_id,product.spec_info,goods.name from
                sdb_ome_products as product left join
                sdb_ome_goods as goods on goods.goods_id = product.goods_id where product.bn= '".$row['product_bn']."' ";
                $row_product = $this->db->selectrow($sql1);
                if($row_product && $row_product['goods_id']>0){
                    $data['goods_name'] = $row_product['name'];
    ...
    public function _filter($filter,$tableAlias=null,$baseWhere=null){
        ...
        if(isset($filter['goods_name']) && $filter['goods_name']){
            $where[] = " p.name like '".$filter['goods_name']."%'";
...
///////////////////////////////////////////////////
    ...
    public function getExportDataByCustom($fields, $filter, $has_detail, $curr_sheet, $start, $end){
        ...
        $datas = array();
        $sql = "select saleitem.name,saleitem.spec_name as goods_specinfo,sales.branch_id,sales.shop_id,sales.order_id,sales.sale_id as sale_id,sales.iostock_bn,saleitem.bn as product_bn,saleitem.nums as buycount,saleitem.sales_amount as sale_amount,sales.sale_time as createtime
                from sdb_ome_sales_items as saleitem left join
                sdb_ome_sales as sales on saleitem.sale_id = sales.sale_id left join sdb_ome_products p ON saleitem.product_id = p.product_id left join sdb_ome_goods as g on p.goods_id=g.goods_id
         ...
         if($rows){
            foreach($rows as $row){
                ...
                if($row_product && $row_product['goods_id']>0){
                    ...
                    $sql2 = "select brand.brand_name from sdb_ome_brand as brand left join sdb_ome_goods as goods on goods.brand_id = brand.brand_id where goods.goods_id='".$row_product['goods_id']."'";
                    $row_brand = $this->db->selectrow($sql2);
                    $data['brand_name'] = $row_brand['brand_name'] ? $row_brand['brand_name'] : '-';
