...
class omeanalysts_mdl_ome_products extends dbeav_model{
    ...
    public function get_products($filter=null){
        ...
        #重新改写原来货品销售情况的统计，和getList那里,保持一致
        $sales_sql = 'SELECT
                        SI.bn
        ...
        if((!$filter['_sale_amount_search'])&&(!$filter['_sale_num_search'])){
            $sales_sql .= ' GROUP BY SI.bn order by null';
        }
        $reship_total_amount = 0;
        $total_reship_num = 0;
        $aftersale_cost_amount = 0;
        
        $aftersale_cost_amount_count    = 0;#重新计算_售后商品成本之和 ExBOY
        
        $rows = $this->db->select($sales_sql);
        foreach($rows as $key=>$val)
        {
            $bn_reship_num    = 0;#货号的退货数量 ExBOY
            
            $sql = "select p.cost, sai.price, sai.num as total_reship_num 
                    from sdb_sales_aftersale_items sai
                    LEFT JOIN sdb_sales_aftersale sa on sai.aftersale_id = sa.aftersale_id
                    LEFT JOIN sdb_ome_sales as S on S.order_id=sa.order_id
                    LEFT JOIN sdb_ome_products p on p.product_id=sai.product_id
                    where ".$this->rfilter($filter)." and sai.bn = '".addslashes($val['bn'])."'";
            
            $row = $this->db->select($sql);
            if(!empty($row))
            {
                foreach ($row as $key_j => $val_j)
                {
                    $aftersale_cost_amount += $val_j['total_reship_num'] * $val_j['cost'];
                    $reship_total_amount += ($val_j['total_reship_num'] * $val_j['price']);
                    $total_reship_num    += $val_j['total_reship_num'];
                    
                    $bn_reship_num    += $val_j['total_reship_num'];
                }
            }
            
            #重新计算_售后商品成本之和 ExBOY
            if((!$filter['_sale_amount_search'])&&(!$filter['_sale_num_search']) && $bn_reship_num > 0)
            {
                $sql    = 'SELECT SI.cost, SI.nums FROM sdb_ome_sales_items SI LEFT JOIN sdb_ome_sales S ON SI.sale_id = S.sale_id 
                          WHERE '.$this->_filter($filter) . ' AND SI.bn = "'.addslashes($val['bn']).'"';
                $row = $this->db->select($sql);
                
                $temp_cost    = array();
                foreach ($row as $key_j => $val_j)
                {
                    $temp_cost['cost']    += ($val_j['cost'] * $val_j['nums']);
                    $temp_cost['nums']    += $val_j['nums'];
                }
                $temp_cost_amount        = $temp_cost['cost'] / $temp_cost['nums'];#多个相同货号_成本平均值
                
                $aftersale_cost_amount_count    += ($temp_cost_amount * $bn_reship_num);#退货的总成本
            }
        }
        ...
        if((!$filter['_sale_amount_search'])&&(!$filter['_sale_num_search']))
        {
            $aftersale_cost_amount    = $aftersale_cost_amount_count;
        }
        
        $data['reship_nums']      = $total_reship_num;
        $data['reship_amounts']   = $reship_total_amount;
        
        //销售毛利 = 销售额-退货总额-总成本(销售成本-售后商品成本之和)
        $data['gross_sales']      = $data['sale_amount']-$reship_total_amount-($salestat['cost_amount']-$aftersale_cost_amount);//$salestat['gross_sales'] - $data['reship_amounts'];
...
/*                  $rows[$key]['obj_type'] = 'normal';
                ...
                if((!$filter['_sale_amount_search'])&&(!$filter['_sale_num_search']))
                {
                    ...
                    $val['aftersale_cost_amount']    = $temp_cost['cost'] / $temp_cost['nums'];
                }
