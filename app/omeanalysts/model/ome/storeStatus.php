...
class omeanalysts_mdl_ome_storeStatus extends dbeav_model{
    ...
    public function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
        ...
        */
        // 周转率
        $sql = sprintf(' SELECT bpsd.product_id,bpsd.branch_id,sum(bpsd.sales_nums) AS sales_nums,ob.name as branch_name,op.bn,op.name,sob.brand_name,IF((cast(sobp.store as signed)-cast(sobp.store_freeze as signed))<0,0,cast(sobp.store as signed)-cast(sobp.store_freeze as signed)) as sale_store FROM `sdb_omeanalysts_sale_products` AS bpsd left join sdb_ome_branch as ob on bpsd.branch_id = ob.branch_id left join sdb_ome_products as op on bpsd.product_id = op.product_id left join sdb_ome_goods as g on op.goods_id = g.goods_id left join sdb_ome_brand as sob on g.brand_id = sob.brand_id left join sdb_ome_branch_product as sobp on (bpsd.branch_id = sobp.branch_id and bpsd.product_id = sobp.product_id) WHERE bpsd.sales_time>=\'%s\' AND bpsd.sales_time<=\'%s\' %s GROUP BY bpsd.branch_id,bpsd.product_id',$start_time,$end_time,$where);
