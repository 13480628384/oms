...
class omeanalysts_mdl_ome_aftersale extends dbeav_model{
    ...
    public function getlist($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
        ...
        $returnBrids = $oReTurn->getList('return_id,branch_id',array('return_id|in'=>$returnids));
        $returnBranchIds = array();
        $returnBranchs = array();
        foreach($returnBrids as $v) {
            if(in_array($v['branch_id'], $returnBranchIds[$v['return_id']])) {
                continue;
            }
            $returnBranchIds[$v['return_id']][] = $v['branch_id'];
            $returnBranchs[$v['return_id']] .= ($returnBranchs[$v['return_id']] ? '&' : '').$branchs[$v['branch_id']];
        }

        foreach($rows as $k=>$v){
            $rows[$k]['branch_id'] = $branchs[$v['branch_id']];
            $rows[$k]['branch_return'] = $returnBranchs[$v['return_id']];
