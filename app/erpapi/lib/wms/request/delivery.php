...
class erpapi_wms_request_delivery extends erpapi_wms_request_abstract
{
    ...
    protected function _format_delivery_create_params($sdf)
    {
        ...
        $invoice_issued_id = app::get('wmsmgr')->getConf('invoice_issued_id'.$this->__channelObj->wms['channel_id']);
        if ($invoice_issued_id){
            $params['invoice_issued_id']  = app::get('wmsmgr')->getConf('invoice_issued_id'.$this->__channelObj->wms['channel_id']);
        }
