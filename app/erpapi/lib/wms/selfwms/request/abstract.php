...
abstract class erpapi_wms_selfwms_request_abstract 
{
    ...
    public function delivery_create($sdf){
        ...
        $consoleDlySendObj = app::get('console')->model('delivery_send');
        $failApiModel = app::get('erpapi')->model('api_fail');
        ...
        if ($rs['rsp'] == 'fail'){
            ...
            if($msg == $chk_msg){
                ...
                $consoleDlySendObj->update_send_status($deliverys['delivery_id'],'succ',$msg);
            }else{
                $this->save_api_fail($sdf['outer_delivery_bn'],'delivery',WMS_SALEORDER_CREATE,$msg);
                $consoleDlySendObj->update_send_status($deliverys['delivery_id'],'fail',$msg);
            }
