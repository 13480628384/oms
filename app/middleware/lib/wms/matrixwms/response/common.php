...
*/
class middleware_wms_matrixwms_response_common extends middleware_wms_abstract{
    ...
    */
    public function response($method,&$params){
        ...
        #参数格式化
        if ($params['wms_id']) {
            $channel_type = $this->getAdapterTypeByWmsId($params['wms_id']);
            if ( in_array($channel_type,array('ilc'))) {
